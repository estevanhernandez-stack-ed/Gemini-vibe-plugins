...
- **Generates onboarding only when it earns one.** When the verdict is `build`, `/vibe-walk-walk` runs five interview gates then emits a drop-in, instrumented, replayable Driver.js tour — you own the emitted code, same model as shadcn.
...
2. Antigravity auto-discovers:
   - **Workflows** from `.agent/workflows/*.md` — slash-invocable (`/vibe-walk`, `/vibe-walk-discover`, `/vibe-walk-walk`, etc.).
   - **Skills** from `.agent/skills/*/SKILL.md` — semantically loaded when relevant (the internal `vibe-walk-session-logger`, `vibe-walk-friction-logger`, and the `vibe-walk-guide` reference detail).
...
```
/vibe-walk-bootstrap (first run) → /vibe-walk-discover (Phase 1) → /vibe-walk-walk (Phase 1.5 + Phase 2)
```
...
|---|---|
| `/vibe-walk` | Bare router — reads project state, recommends the next step, asks before launching. First run hands off to `/vibe-walk-bootstrap`. |
| `/vibe-walk-bootstrap` | First-run setup — classifies the app, resolves the likely substrate, detects existing onboarding, writes `.vibe-walk/config.json`. |
...
| `/vibe-walk-walk` | Phase 1.5 interview gates + Phase 2 build (anchors → module → analytics → trigger wiring). Requires a `build` verdict. |
| `/vibe-walk-vitals` | Read-only structural self-test — checks `agent.json`, all six workflows + three skills, scripts, guide references, friction-trigger wiring. |
| `/vibe-walk-evolve` | L3 self-evolution — reads session + friction logs and proposes improvements to the plugin. Never auto-applies. |

## State files (per host project)

- `.vibe-walk/config.json` — app category, framework, likely substrate, existing-onboarding inventory (written by `/vibe-walk-bootstrap`).
- `.vibe-walk/discovery.json` — surface inventory, ranked shortlist, named aha moment, and the **verdict** (written by `/vibe-walk-discover`; authoritative).
- `.vibe-walk/build-plan.json` — the resolved five-gate answers + substrate decision (written by `/vibe-walk-walk`).
