---
description: "Run when the user says `/vibe-walk-evolve` and wants vibe-walk to reflect on past sessions and propose improvements to itself. Reads ~/.gemini/antigravity/data/vibe-walk/ session logs + friction.jsonl, weights findings, and writes proposed SKILL/verdict/convention edits to docs/proposed-changes.md in the vibe-walk repo. Never auto-applies. L3 self-evolution."
---

# /vibe-walk-evolve — propose improvements from observed use

Sherpa persona, posture, and hard rules are always-on via `AGENTS.md`. The build conventions (D1–D6) and friction-trigger map are in the `vibe-walk-guide` skill (`.agent/skills/vibe-walk-guide/SKILL.md`) — load them to ground any proposed convention or trigger change. Then follow this workflow.

Named `/vibe-walk-evolve` (the plugin-prefix namespacing convention — every non-router workflow is `/<plugin>-<cmd>`, and `evolve-*` collapses to `/<plugin>-evolve`) so it never collides with the other vibe-* plugins' evolve workflows in Antigravity's flat slash namespace. Implements Level 3 of the Self-Evolving Plugin Framework.
...
3. **Propose edits.** Map findings to concrete changes — a verdict-condition tweak, a substrate-tree adjustment, a copy-guardrail change, a convention update, a workflow clarification. **Name the real port target for every proposed edit** (this is not a blind path — a thing that became a workflow lives in a different dir than a thing that stayed a skill):
   ...
   - The D1–D6 conventions → `.agent/skills/vibe-walk-guide/references/conventions.md`.
   - The friction-trigger map → `.agent/skills/vibe-walk-guide/references/friction-triggers.md`.
   - A logger's behavior → `.agent/skills/vibe-walk-session-logger/SKILL.md` or `.agent/skills/vibe-walk-friction-logger/SKILL.md`.
...
- Session log: the `vibe-walk-session-logger` skill (`.agent/skills/vibe-walk-session-logger/SKILL.md`)
- Friction log: the `vibe-walk-friction-logger` skill (`.agent/skills/vibe-walk-friction-logger/SKILL.md`)
