---
description: "Run when the user says /bootstrap, 'set up vibe-walk', or 'init vibe-walk' — also invoked by the /vibe-walk bare router on first run. Classifies the host app, infers the framework + likely substrate, detects existing onboarding, and writes .vibe-walk/config.json. Idempotent — re-runnable to refresh stale config."
---
...
2. **Resolve the likely substrate** by running the substrate decision tree (see `.agent/skills/guide/references/conventions.md` D3). This is provisional — `walk` confirms it at the gate.
...
At workflow start, call `session-logger.start("bootstrap", <project_dir>)` (see the `session-logger` skill (`.agent/skills/session-logger/SKILL.md`)) for the sessionUUID; hold it for the run; pass it to every `friction-logger.log()`. At workflow end, call `session-logger.end({...})` with `verdict: null` (bootstrap doesn't produce a verdict). Honor the friction trigger map at `.agent/skills/guide/references/friction-triggers.md` — section `/bootstrap`.
...
Want the surface read + tour verdict? Run /discover.
```
...
- Router: `/vibe-walk`
- Conventions (substrate tree D3): `.agent/skills/guide/references/conventions.md`
