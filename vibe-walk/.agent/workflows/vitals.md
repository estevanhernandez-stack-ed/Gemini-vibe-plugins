---
description: "Run when the user says `/vitals` or wants a structural integrity check on the vibe-walk install. Runs read-only structural checks and reports findings in a banner-style report. Implements Pattern #8 (Plugin Self-Test) from the Self-Evolving Plugin Framework. Read-only — no auto-fix in this release."
---

# /vitals — structural self-test

Slash command `/vitals`. Runs **read-only** structural checks against the installed plugin files, reports findings in a banner-style report with per-check status (✓ pass, ⚠ warn, ✗ fail), and prints a summary line. No writes, no auto-fix in this release.
...
Sherpa persona and posture are always-on via `AGENTS.md`. Vitals applies the Sherpa voice to the opening line only — the report body is neutral. (This workflow reads the `guide` skill's references only to confirm they exist; it doesn't load their content.)

## Session Logging

Call `session-logger.start("vitals", project_dir_basename)` at workflow start. Hold the returned `sessionUUID`. At workflow end, call `session-logger.end()` with:
...
| Guide references | `.agent/skills/guide/references/*.md` |
...
| Friction triggers doc | `.agent/skills/guide/references/friction-triggers.md` |
...
```
vibe-walk.md    bootstrap.md    discover.md    walk.md    vitals.md    evolve-walk.md
```
...
```
guide    session-logger    friction-logger
```
...
**(a) Read.** Check for:
- `.agent/skills/guide/references/conventions.md`
- `.agent/skills/guide/references/friction-triggers.md`

**(b) Evaluate.** Record which are missing.

**(c) Report.**
- ✓ pass: both present. Include: `2/2 guide references present`.
- ✗ fail: one or more missing. List each as `.agent/skills/guide/references/<name> — missing`.
...
**(a) Read.** Open `.agent/skills/guide/references/friction-triggers.md`. Parse section headings (lines starting with `##`).
...
```
Re-run /vitals any time to re-check. For structural proposals, see /evolve-walk.
```
