...
2. If absent → tell the builder to run `/discover` first.
...
When every item is resolved, re-run /walk. The gates will not re-run;
...
For `/walk`, fire `friction-logger.log()` when:
