...
- A command (`/discover`, `/walk`, `/bootstrap`) is about to **make a build decision** — substrate, anchor contract, step cap, analytics schema — and needs the D1–D6 conventions.
...
- [`references/friction-triggers.md`](references/friction-triggers.md) — where each command invokes `friction-logger.log()`, with the friction type and confidence (per Pattern #6, consumed by the friction-logger skill).

## Self-evolving framework — the two internal skills

Every command (`/bootstrap`, `/discover`, `/walk`, `/vitals`) invokes:
- [`../session-logger/SKILL.md`](../session-logger/SKILL.md) — sentinel + terminal session entries, paired by sessionUUID, written to `~/.gemini/antigravity/data/vibe-walk/sessions/<date>.jsonl`.
- [`../friction-logger/SKILL.md`](../friction-logger/SKILL.md) — append-only friction entries written to `~/.gemini/antigravity/data/vibe-walk/friction.jsonl`.

The `/evolve-walk` workflow reads both logs and proposes plugin improvements. It never auto-applies.
