---
name: vibe-walk-friction-logger
...
# vibe-walk-friction-logger — append-only friction capture

Internal SKILL. Not user-invocable. Commands call `log()` at the trigger points in [`../vibe-walk-guide/references/friction-triggers.md`](../vibe-walk-guide/references/friction-triggers.md). At startup, `bootstrap` (and any command) may call `detect_orphans()` to convert abandoned sessions into friction signal.
...
- **schema_version / timestamp / plugin / plugin_version / command / project_dir** — same semantics as the vibe-walk-session-logger.
- **sessionUUID** — the value from `vibe-walk-session-logger.start()` for this run. Required so friction clusters under its session.
...
Called once at command startup, after `vibe-walk-session-logger.start()` has written this run's sentinel.
...
Friction entries are the highest-signal input to **Level 3** — `/vibe-walk-evolve` weights them to propose improvements. The most load-bearing type is `verdict_overridden`: it tells us when the plugin's earn-the-tour verdict disagrees with the builder, which is exactly the judgment the plugin most needs to calibrate.

## Cross-references

- Sibling: the `vibe-walk-session-logger` skill (`.agent/skills/vibe-walk-session-logger/SKILL.md`)
- Trigger map: [`../vibe-walk-guide/references/friction-triggers.md`](../vibe-walk-guide/references/friction-triggers.md)
- Reader: `/vibe-walk-evolve`
