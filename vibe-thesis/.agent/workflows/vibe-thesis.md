...
ON_INVOCATION:
  ...
  marker_present = grep -q "VIBE_THESIS_MARKER:" AGENTS.md (if AGENTS.md exists)
...
The workflow needs to know where the plugin's bundled assets live to reach
`templates/full/`, `templates/overlay/`, and `examples/demo-article/`. In the
Antigravity port these ship under the plugin's `.agent/` directory:
`.agent/templates/full/`, `.agent/templates/overlay/`,
`.agent/examples/demo-article/`. Resolve `PLUGIN_DIR` to the workspace's
`.agent/` directory:

```bash
# This workflow lives at <workspace>/.agent/workflows/vibe-thesis.md.
# PLUGIN_DIR is the .agent/ dir one level up from workflows/.
# Antigravity copies .agent/* into the target project at install; the bundled
# assets land at .agent/templates/ and .agent/examples/.
PLUGIN_DIR="$(pwd)/.agent"
test -d "$PLUGIN_DIR/templates/full" || PLUGIN_DIR=".agent"
...
test -d "$PLUGIN_DIR/templates/full" || {
  ...
  echo "Plugin install may be incomplete; re-run the Antigravity install (have the agent re-copy .agent/* from the vibe-thesis port repo)." >&2
  exit 1
}
```

Antigravity's agentic install copies the port's `.agent/*` into the target
project, so the bundled templates land at `<workspace>/.agent/templates/`.
When emitting `Bash(...)` invocations, use `${PLUGIN_DIR}` consistently as the
prefix. **Never** hardcode an install path — resolve `.agent/` relative to the
workspace root.
...
- **Path A (`gh_ok=false` OR user override):**
  ...
  - **Template carry note (port).** The bundled `templates/full/` is the
    ThesisStudio scaffold, carried **verbatim** — it ships ThesisStudio's
    own `CLAUDE.md` (with the `VIBE_THESIS_MARKER` stanza baked in) and
    `.claude/skills/`. After copying, the port treats the scaffolded
    project's rules file as `AGENTS.md`: copy the template's `CLAUDE.md`
    to `AGENTS.md` (`cp CLAUDE.md AGENTS.md`) so the marker + project rules
    live where Antigravity reads them. The verbatim `CLAUDE.md` stays for
    cross-tool compatibility; `AGENTS.md` is the live rules file the
    workflows read and write. No inject-marker step needed — the stanza is
...
directory. (Antigravity reads project-local skills from `.agent/skills/`;
the ThesisStudio scaffold ships them under `.claude/skills/` — if running
under Antigravity, mirror the bootstrap/merge-authors/lay-translator/
research-integrate skill dirs into the project's `.agent/skills/` so they're
discoverable, or invoke them directly by following their SKILL.md.)

Dispatch the bootstrap skill. It runs its own 6-question interview (title,
author, mode, three pillars, citation style, license) and rewrites
placeholders across the project rules file (`AGENTS.md`), README.md, LICENSE,
package.json.
...
`## VOICE SYNTHESIS` block to AGENTS.md.
...
Try /vibe-thesis-status to see the project state at a glance.
```
...
`/vibe-thesis-render` for diagnostic hints.
...
| User says | Dispatch |
|---|---|
| "render the PDF" / "give me the HTML" / "produce all formats" | `/vibe-thesis-render <format>` |
| "what's the state" / "where am I" / "project status" | `/vibe-thesis-status` |
...
*"Before rendering, want me to run the synthesis guard first? It catches
self-review tone (inflationary language, self-praise framings) before the PDF
goes out. Pass `--guard` to /vibe-thesis-render to integrate this into
...
- User says "scaffold for me" but already has `gh repo create --template`
  ...
  `[YOUR THESIS TITLE]` in AGENTS.md AND no marker) and switch to overlay-only
