---
description: "Apply the project's voice profile to a draft markdown file. Multi-pass rewrite (micro / rhythm / macro / exemplar-comparison) preserving claims and structure. Output to <draft>.smoothed.md. The apply step in the voice pipeline (capture → apply → lint)."
---
...
1. Current directory contains a `VIBE_THESIS_MARKER` stanza in `AGENTS.md`.
2. AGENTS.md contains a `## VOICE SYNTHESIS` block written by
   `/vibe-thesis-voice`.
...
1. **Marker check.** If `VIBE_THESIS_MARKER` stanza is missing from
   `AGENTS.md`, refuse politely. *"This isn't a vibe-thesis project.
   Smooth is scoped to projects with a captured voice profile."*

2. **Voice block check.** Grep for `## VOICE SYNTHESIS` in AGENTS.md.
...
Each step stands alone — smooth doesn't invoke voice or guard, and
