---
description: "Author voice synthesis. Base mode: interview to name 2-4 author voices + 2-4 field experts + a synthesis ratio. Extend mode: add written exemplars, narrative (transcribed spoken) samples, and forced-choice extracted micro/macro rules — the layers /vibe-thesis-smooth needs to do real transformation. Writes a VOICE SYNTHESIS block to AGENTS.md."
---
...
- **On-demand.** User runs `/vibe-thesis-voice` to revise the voice mid-project.
  Idempotent — overwrites the existing `## VOICE SYNTHESIS` block in AGENTS.md.
...
> not sure, I can suggest based on `<thesis topic from AGENTS.md / scope.md>`."
...
Append (or replace, in revise mode) the following block to `AGENTS.md`. Insert
...
- **Channel A — written exemplars.** User points at 2-3 passages that
  "sound most like you." The workflow auto-extracts micro candidates (verb
...
Use `Edit` to write this back to `AGENTS.md`. Replace the existing
...
4. Replace the existing block in-place using `Edit` (not `Write` — preserve
   the rest of AGENTS.md byte-for-byte).
...
- User can't think of contemporary experts and declines suggestions: write
  ...
  to add later.` Keep timeless voices; warn that `/vibe-thesis-guard` will be
...
- User's AGENTS.md is in a non-standard state (no marker, no pillar block):
