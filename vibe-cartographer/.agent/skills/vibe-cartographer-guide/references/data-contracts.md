...
|------|------|--------|--------|--------|
| Unified profile | `~/.gemini/profiles/builder.json` | JSON object | [`../schemas/builder-profile.schema.json`](../schemas/builder-profile.schema.json) | atomic-write |
| Friction log | `~/.gemini/antigravity/data/vibe-cartographer-friction.jsonl` | JSONL | [`../schemas/friction.schema.json`](../schemas/friction.schema.json) | atomic-append |
| Friction calibration | `~/.gemini/antigravity/data/vibe-cartographer-friction.calibration.jsonl` | JSONL | [`../schemas/friction-calibration.schema.json`](../schemas/friction-calibration.schema.json) | atomic-append |
| Session log | `~/.gemini/antigravity/data/vibe-cartographer/sessions/<date>.jsonl` | JSONL | [`../schemas/session-log.schema.json`](../schemas/session-log.schema.json) | atomic-append |
...
6. **Removing a plugin removes only `plugins.<name>`.** Uninstalling Vibe Cartographer must not touch `shared.*` or any other plugins' blocks. `/vibe-cartographer-vitals` and `/vibe-cartographer-evolve` rely on this.
...
Default TTLs (set by `vibe-cartographer-decay.stamp()` on first stamp if missing):
...
| Type | Meaning |
|------|---------|
| `command_abandoned` | Sentinel session-log entry without a terminal pair after 24h. Emitted by `vibe-cartographer-friction-logger.detect_orphans()`. |
...
`confidence` is one of `high`, `medium`, `low`. `/vibe-cartographer-evolve` weighting: `high=1.0`, `medium=0.6`, `low=0.3`. Calibration entries can zero an entry's weight.

When in doubt, **don't log** (Spec Key Decision #6). False positives poison `/vibe-cartographer-evolve`; false negatives are recoverable through the `/vibe-cartographer-reflect` calibration check-in.
...
JSONL, append-only. Written by `/vibe-cartographer-reflect`'s calibration check-in. See [`../schemas/friction-calibration.schema.json`](../schemas/friction-calibration.schema.json).
...
- `false_positive` — the referenced friction entry wasn't real. `/vibe-cartographer-evolve` multiplies its weight by 0.0 (effectively removes it).
...
**Sentinel entry** — written by `vibe-cartographer-session-logger.start(command, project_dir)` at command invocation. Required fields: `schema_version`, `timestamp`, `plugin`, `plugin_version`, `command`, `project_dir`, `sessionUUID`, `outcome: "in_progress"`. Plus optional `project_id`, `mode`, `persona`. The `outcome: "in_progress"` value is the literal sentinel marker — `vibe-cartographer-friction-logger.detect_orphans()` looks for sentinels older than 24h with no matching terminal entry.

**Terminal entry** — written at workflow end. Same identifying fields plus `outcome: "completed" | "abandoned" | "error" | "partial"`, `user_pushback` (bool), `friction_notes` (array of strings), `key_decisions` (array of strings), `artifact_generated` (string or null), `complements_invoked` (array of strings).
...
At terminal write time, `vibe-cartographer-session-logger` ALSO updates `plugins.vibe-cartographer._meta.last_seen_complements` in the unified profile via atomic-write — that's the only profile mutation tied to the session log.

---

## Atomic-write protocol

**When to use:** any write to the unified profile JSON (`~/.gemini/profiles/builder.json`). Anywhere you need to rewrite an entire file and need to guarantee that a reader either sees the previous full version or the new full version, never a half-written file.

**How:** pipe the new JSON object to `node .agent/scripts/atomic-write-json.js <target-path>`. The script:
...
- Step 3 or 4 fails → original file unchanged, `.tmp` debris left behind. `/vibe-cartographer-vitals` check #7 detects and offers cleanup `(e)`.
...
**How:** pipe one JSON object to `node .agent/scripts/atomic-append-jsonl.js <target-path>`. The script:
...
- Concurrent-write corruption (Windows edge case) → malformed lines detected by `/vibe-cartographer-vitals` check #8 with auto-fix `(f)` repair (rewrites file dropping malformed lines, backs up to `.bak`).

---

## Adding a new data file

When adding a new file under `~/.gemini/antigravity/data/vibe-cartographer/` or a new namespace block to the profile:
...
4. Add a `/vibe-cartographer-vitals` check if the file's integrity is load-bearing.
