---
name: vibe-cartographer-decay
description: "Internal SKILL — not a user workflow. Profile-decay engine for Vibe Cartographer. Invoked by /vibe-cartographer-onboard at workflow start to gently re-validate stale fields in the unified builder profile (~/.gemini/profiles/builder.json). Implements Pattern #4 (Memory Decay and Refresh) from the Self-Evolving Plugin Framework."
---

# vibe-cartographer-decay — Profile Decay Engine

Internal SKILL. Not a user-invocable slash command. Loaded by `/vibe-cartographer-onboard` only.
...
- **Data contract:** `skills/vibe-cartographer-guide/references/data-contracts.md` — read the "Unified Profile" and "Decay metadata" sections.
- **Schema:** `skills/vibe-cartographer-guide/schemas/builder-profile.schema.json` — `_meta` blocks live under each namespace (`shared._meta` and `plugins.<name>._meta`) keyed by dotted field path. Per-entry shape: `{ last_confirmed, stale, ttl_days }`.
...
- **Atomic writes only:** all profile writes go through `node .agent/scripts/atomic-write-json.js ~/.gemini/profiles/builder.json` (stdin = full profile JSON). Never write the profile inline.
...
When more than one field is stale, surface the highest-priority one (only one decay prompt per `/vibe-cartographer-onboard` run):
...
1. **Read the profile.** `cat ~/.gemini/profiles/builder.json` (or equivalent). If the file does not exist, return `null` — there is nothing to decay.
...
The returned value is a string field path the caller (`/vibe-cartographer-onboard`) uses to phrase the confirmation question and pass back to `stamp()`.
...
7. **Atomic write.** Pipe the full profile JSON to `node .agent/scripts/atomic-write-json.js ~/.gemini/profiles/builder.json`. On non-zero exit, surface the stderr message to the caller. Do not retry — the caller owns retry policy.

`stamp()` does **not** mutate the field's value. Updating the value (because the user said "actually I switched to professor") is the caller's responsibility — `/vibe-cartographer-onboard` writes the new value, then calls `stamp()` to refresh the timestamp.
...
The migration runs **silently** the first time `/vibe-cartographer-onboard` (or any caller) loads the profile under 1.5.0:
...
If the migration write fails (atomic-write returns non-zero), surface the error and skip the decay check for this run. Do not block `/vibe-cartographer-onboard`.

## Invocation Order in `/vibe-cartographer-onboard`

`/vibe-cartographer-onboard` calls in this sequence at workflow start:

1. (Item 5 ships this) write the vibe-cartographer-session-logger sentinel entry.
...
The decay SKILL doesn't render the prompt itself — `/vibe-cartographer-onboard` does. But this is the right place to anchor the tone:
...
- **Profile file missing:** `check_decay()` returns `null`. `stamp()` returns without writing. `/vibe-cartographer-onboard` proceeds as a new-builder flow.
- **Profile JSON malformed:** surface the parse error to the caller. Do not attempt to fix. `/vibe-cartographer-vitals` check #3 owns schema repair.
- **Atomic-write fails:** surface stderr from `atomic-write-json.js`. `/vibe-cartographer-onboard` should warn in `process-notes.md` and continue — the decay subsystem is best-effort, not blocking.
...
The unified builder profile is durable across projects, plugins, and time. Without decay, a "first-time coder" tag from 18 months ago haunts every `/vibe-cartographer-onboard`. With decay, the profile self-corrects — but only with the user in the loop. The `decay_disabled: true` flag is the escape hatch for users who want their profile frozen forever.
