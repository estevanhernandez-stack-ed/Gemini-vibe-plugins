---
name: vibe-cartographer-session-logger
description: "Internal SKILL — not a user workflow. Two-phase append-only session log for Vibe Cartographer: a sentinel entry at workflow start (outcome=in_progress) and a terminal entry at workflow end, paired by sessionUUID. Also updates plugins.vibe-cartographer._meta.last_seen_complements on the unified profile at terminal time. Referenced by the vibe-cartographer-guide skill; invoked by every workflow at start and end. Part of Level 2 (session memory) of the Self-Evolving Plugin Framework."
---

# vibe-cartographer-session-logger — Sentinel + Terminal Session Log

Internal SKILL. Not a user-invocable slash command. Every Vibe Cartographer command calls `start()` at invocation and the terminal-append procedure at completion. The two entries share a `sessionUUID` so `vibe-cartographer-friction-logger.detect_orphans()` can pair them.

## Before You Start

- **Data contract:** [`../vibe-cartographer-guide/references/data-contracts.md`](../vibe-cartographer-guide/references/data-contracts.md) — read the "Session log" section. The sentinel vs terminal shapes, required fields, and sessionUUID pairing contract live there.
- **Session log schema:** [`../vibe-cartographer-guide/schemas/session-log.schema.json`](../vibe-cartographer-guide/schemas/session-log.schema.json) — JSON Schema Draft-07. The schema's `oneOf` splits sentinel (outcome=in_progress) from terminal (outcome in completed/abandoned/error/partial). Every write validates against this.
- **Unified profile schema:** [`../vibe-cartographer-guide/schemas/builder-profile.schema.json`](../vibe-cartographer-guide/schemas/builder-profile.schema.json) — the `plugins.vibe-cartographer._meta.last_seen_complements` sub-shape (list / timestamp / previous_diff_count / notable_change_at) is defined here. Pattern #11 namespace isolation: this SKILL writes ONLY inside `plugins.vibe-cartographer._meta`, never into `shared.*` or any other plugin's namespace.
- **Atomic protocol:** all session log writes go through `node .agent/scripts/atomic-append-jsonl.js`; all profile writes go through `node .agent/scripts/atomic-write-json.js`. Never `>>` from a shell.
- **Orphan pairing:** the sentinel's `sessionUUID` is the load-bearing field. `vibe-cartographer-friction-logger.detect_orphans()` scans 7 days of session files looking for sentinels whose `(command, project_dir, sessionUUID)` has no matching terminal entry.

## Where the Log Lives

`~/.gemini/antigravity/data/vibe-cartographer/sessions/<YYYY-MM-DD>.jsonl`
...
**Legacy logs** from v0.5.0 and earlier live at `~/.gemini/antigravity/data/app-project-readiness/sessions/`. These are preserved untouched — append-only history is never migrated. Future recall commands read both locations.
...
- **plugin_version** — read from `plugins/vibe-cartographer/.agent/agent.json`. If you can't determine it, use `"unknown"`.
...
- **friction_notes** — array of short strings. Human-facing recap for `/vibe-cartographer-reflect`. The actual structured friction signal goes to `friction.jsonl` via `vibe-cartographer-friction-logger.log()`.
...
2. **Determine audit fields.**
   ...
   - `plugin: "vibe-cartographer"`.
   - `plugin_version`: read from `plugins/vibe-cartographer/.agent/agent.json`'s `"version"`. Fall back to `"unknown"`.
   - `project_id`: from the 626 Labs session bind if available; otherwise omit.
   - `mode` / `persona`: read from the unified profile at `~/.gemini/profiles/builder.json`. Pass through as-is; `null` if unset.
...
5. **Atomic append.** Pipe the JSON-stringified entry to:
   ```bash
   node .agent/scripts/atomic-append-jsonl.js ~/.gemini/antigravity/data/vibe-cartographer/sessions/<today>.jsonl
   ```
...
**Failure handling:** same as `start()` — session logging is instrumentation. A failed append logs a one-line warning to stderr and the command proceeds to handoff. The user never sees a vibe-cartographer-session-logger error.
...
1. **Read the current unified profile** from `~/.gemini/profiles/builder.json`. If the file doesn't exist, start from the minimal shape `{ "schema_version": 1, "shared": {}, "plugins": {} }`.
...
4. **Compute `notable_change_at`.**
   ...
   - Else: preserve the previous `notable_change_at` value (or `null` if first run). The field is **sticky** — once stamped, it persists across subsequent non-material runs so `/vibe-cartographer-evolve` and `/vibe-cartographer-vitals` can see "there was a material env shift at T." A later material shift overwrites it with the newer timestamp.
...
6. **Atomic write.** Pipe the JSON-stringified updated profile to:
   ```bash
   node .agent/scripts/atomic-write-json.js ~/.gemini/profiles/builder.json
   ```
...
1. **`~/.gemini/antigravity/data/vibe-cartographer/sessions/<date>.jsonl`** — session log file (plugin-owned directory, append-only).
2. **`~/.gemini/profiles/builder.json`** — ONLY inside `plugins.vibe-cartographer._meta.last_seen_complements`. Never `shared.*`. Never `plugins.<other-name>.*`. The step-7 invariant check above exists specifically to prevent accidental cross-namespace writes.

`shared.*` updates are the job of the dedicated `update_shared_profile` step on commands like `/vibe-cartographer-onboard` and `/vibe-cartographer-reflect`, not this SKILL. `last_seen_complements` is Cart-specific telemetry — it lives under the plugin's own block because Vibe Doc, Vibe Sec, and friends will adopt Pattern #4 with their own complement snapshots later. A top-level `_meta.last_seen_complements` would create cross-plugin write coordination problems (see Key Technical Decision #1 in `spec.md`).
...
The session log is raw material for **Level 3** of the Self-Evolving Plugin Framework. `/vibe-cartographer-evolve` reads these entries (alongside `friction.jsonl`) to propose plugin improvements based on observed patterns.

The **sentinel pattern** (Story 2.2) lets `vibe-cartographer-friction-logger.detect_orphans()` distinguish "user abandoned the command" from "command never ran" — abandonment is friction signal worth surfacing; non-execution isn't.

The **`last_seen_complements` snapshot** (Story 4.2) gives `/vibe-cartographer-evolve` and `/vibe-cartographer-vitals` a way to detect material environment shifts. When two or more complements appear or disappear between runs, `notable_change_at` gets stamped — `/vibe-cartographer-evolve` uses this to reweight complement-rejection patterns (the user may have rejected a complement because a better one just became available, not because the complement is bad).
...
`process-notes.md` at the project level but no session-log entries, leaving `/vibe-cartographer-evolve`
blind to arguably the richest use case of the plugin. The runtime-not-wired banner
(see `skills/vibe-cartographer-guide/SKILL.md > Session Logging`) surfaces this to the builder at
command start. This section specifies the backfill recipe a future
`/reconnect` slash command will implement.

**Input:** project root directory containing `process-notes.md`.

**Parse:** walk top-level headings of the form `## /<command>` (e.g. `## /vibe-cartographer-onboard`,
`## /vibe-cartographer-scope`, ...). For each heading, extract:
