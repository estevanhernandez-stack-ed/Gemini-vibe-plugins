...
This folder contains architecture guidance that the plugin reads during `/vibe-cartographer-onboard` and uses throughout the entire workflow — especially in `/vibe-cartographer-spec` and `/vibe-cartographer-checklist`.

## How It Works

During `/vibe-cartographer-onboard`, the agent asks the builder to point to their architecture docs. These docs tell the agent:
...
The agent reads these docs and uses them as the foundation for all technical decisions in `/vibe-cartographer-spec`, `/vibe-cartographer-checklist`, and `/vibe-cartographer-build`.
...
- Business logic (that's discovered during `/vibe-cartographer-scope` and `/vibe-cartographer-prd`)
- One-off decisions (those happen in `/vibe-cartographer-spec` conversation)
