...
# /vibe-cartographer-friction — Friction Log Inspection

Slash command `/vibe-cartographer-friction`. **Read-only** view into `~/.gemini/antigravity/data/vibe-cartographer-friction.jsonl`. Default output is the last 30 days, grouped by friction type, with confidence indicators. Filters compose as AND.

This is the inspection counterpart to `/vibe-cartographer-evolve` (which consumes friction for proposals) and `/vibe-cartographer-vitals` (which structurally audits the log). `/vibe-cartographer-friction` does not write. It does not propose. It shows you what your past self flagged.

## Before You Start

- **Data contract:** [`.agent/skills/vibe-cartographer-guide/references/data-contracts.md`](.agent/skills/vibe-cartographer-guide/references/data-contracts.md) — read the "Friction log" section. Per-line schema and the seven canonical friction types live there. Filter semantics below rely on those field names.
- **Friction schema:** [`.agent/skills/vibe-cartographer-guide/schemas/friction.schema.json`](.agent/skills/vibe-cartographer-guide/schemas/friction.schema.json) — per-line shape. `friction_type` enum, `confidence` enum, required fields. This SKILL reads but never writes.
- **Friction triggers contract:** [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — the `/vibe-cartographer-friction` section is intentionally empty. `/vibe-cartographer-friction` does **not** call `vibe-cartographer-friction-logger.log()` in 1.5.0. Inspection declines or "nothing interesting" outcomes are the expected mode of interaction, not friction signal. If a future version grows interactive marking (e.g., "mark this entry false positive"), those marks write to `friction.calibration.jsonl`, not to `friction.jsonl`.
- **Output-style reference:** `/vibe-cartographer-vitals` — banner header, indented-two boxed sections, ✓ / ⚠ / · glyph conventions. `/vibe-cartographer-friction` matches that aesthetic so the two commands read as siblings.
- **Session logger interface:** the `vibe-cartographer-session-logger` skill (`.agent/skills/vibe-cartographer-session-logger/SKILL.md`) — `start("friction", <project_dir>)` at invocation returns the sessionUUID for this run; terminal append at workflow end. `/vibe-cartographer-friction` bookends with sentinel + terminal entries like every other command.

## Session Logging

At workflow start, call `vibe-cartographer-session-logger.start("friction", <project_dir>)` to get the sessionUUID. Hold it in memory for the duration of the command.

At workflow end (after the report renders), call the vibe-cartographer-session-logger terminal-append procedure with:
...
- `artifact_generated: null` — `/vibe-cartographer-friction` produces inline output, not a persisted doc.
- `complements_invoked: []` — `/vibe-cartographer-friction` does not defer to Pattern #13 complements.
- `friction_notes: []` — see "Friction Logging" below.
- `key_decisions`: short strings only for notable inspection outcomes a future reader of the session log would want to see without re-running `/vibe-cartographer-friction` (e.g., `"skipped 3 malformed friction.jsonl lines"`, `"no friction entries in last 30 days"`). Routine inspections with a handful of entries do not need a decision line.

## Friction Logging

Reference: [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-friction` is intentionally empty. `/vibe-cartographer-friction` does **not** call `vibe-cartographer-friction-logger.log()` in 1.5.0. Running a read-only inspection and declining to act on what you see is the **expected** mode of interaction, not friction. Logging it would flood `/vibe-cartographer-evolve` with noise about a user simply looking at their own data.
...
Check #6 of `/vibe-cartographer-vitals` explicitly audits that this SKILL declares no `friction_type` in the Friction Logging section, consistent with the documented empty `/vibe-cartographer-friction` table.

## Persona Adaptation

`/vibe-cartographer-friction` is a read-only inspection, not a conversation. Persona applies only to the one-line opening before the report renders. The boxed report body is neutral — the data speaks.

Read `shared.preferences.persona` from `~/.gemini/profiles/builder.json`. Keep the opening to one sentence:
...
All paths `/vibe-cartographer-friction` reads (never writes):
...
| Unified profile | `~/.gemini/profiles/builder.json` (for persona only — optional, missing profile falls back to system default persona) |
...
1. Open `~/.gemini/antigravity/data/vibe-cartographer-friction.jsonl`.
...
2. Read line by line. For each line:
   - Parse as JSON. **Malformed lines are skipped silently** (same convention as `/vibe-cartographer-evolve` — `/vibe-cartographer-vitals` check #8 owns repair). Keep a count of skipped lines for a single footer line (see output format).
...
After the report (or empty-state box), do the vibe-cartographer-session-logger terminal append (see "Session Logging" above). No handoff — `/vibe-cartographer-friction` is terminal, not a step in any sequential chain.

## Output Format

The report is rendered as markdown. Color is conveyed via emoji; box drawing uses Unicode characters matching the `/vibe-cartographer-vitals` banner aesthetic. Everything below is agent output — the evaluator emits it verbatim (with computed values substituted) after the filters apply.

### Banner header

Two-line banner, indented two spaces, with a separator rule. Same shape as `/vibe-cartographer-vitals`:
...
The glyphs match `/vibe-cartographer-vitals`'s pass/warn/info semantics by design — a "✓ high" friction entry is the strongest signal, the same way a `/vibe-cartographer-vitals` ✓ pass is the strongest signal. The middle-dot low-confidence glyph is intentionally quiet so a log full of low-confidence entries doesn't visually overpower a handful of high-confidence ones.
...
**Box width:** target 68 columns for the top/bottom rules so the group boxes render identically across types and match the `/vibe-cartographer-vitals` boxes visually.
...
```
  (<N> malformed lines skipped silently — run /vibe-cartographer-vitals for line-integrity details.)
```

This is the same convention `/vibe-cartographer-evolve` uses: read-only commands never try to repair — repair is `/vibe-cartographer-vitals`'s job (auto-fix f).
...
**Staged.** `~/.gemini/antigravity/data/vibe-cartographer-friction.jsonl` does not exist.
...
4. Below the summary: `(2 malformed lines skipped silently — run /vibe-cartographer-vitals for line-integrity details.)`.
5. Session-logger terminal append with `outcome: "completed"`, `key_decisions: ["skipped 2 malformed friction.jsonl lines"]`.

The skip is **silent** — `/vibe-cartographer-friction` never names line numbers or renders parse errors inline. That surface belongs to `/vibe-cartographer-vitals` check #8, which owns both diagnosis and the auto-fix (f) repair path. Keeping `/vibe-cartographer-friction` hands-off preserves the "inspection, not triage" contract.
...
`/vibe-cartographer-friction` is the minimum that meets both: a filter-and-group renderer with a banner output style consistent with `/vibe-cartographer-vitals`, separate from `/vibe-cartographer-evolve` (which consumes friction for proposals) and separate from `/vibe-cartographer-vitals` (which audits structural integrity). The separation matters — `/vibe-cartographer-evolve` is a heavy "propose edits" conversation, `/vibe-cartographer-vitals` is an integrity sweep, `/vibe-cartographer-friction` is "show me what I flagged." Three different mental models, three different invocation cadences.

The read-only contract is crisp: `/vibe-cartographer-friction` does not write, does not propose, does not classify. It reads, filters, groups, renders. That invariant is what makes `/vibe-cartographer-friction` safe to run at any time — mid-project, between commands, or any time the builder wants to see what past runs recorded.
