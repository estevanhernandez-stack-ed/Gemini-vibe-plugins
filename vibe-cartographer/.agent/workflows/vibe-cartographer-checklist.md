...
# /vibe-cartographer-checklist — Plan Your Build

Coach persona, posture, and the deepening-interview posture are always-on via `AGENTS.md`. The checklist template, the deepening-round mechanics, and the build-mode behavior (step-by-step vs autonomous — locked here, read in `/vibe-cartographer-build`) are in the `vibe-cartographer-guide` skill. Then follow this workflow.
...
Read `shared.preferences.persona` from `~/.gemini/profiles/builder.json`. Your voice throughout this entire workflow — how you discuss sequencing, present build mode options, and gut-check the plan — must reflect the builder's chosen persona. See the `vibe-cartographer-guide` skill's Persona reference for the full table. Key behaviors per persona during /vibe-cartographer-checklist:
...
`docs/spec.md` and `docs/prd.md` must exist. If either is missing: "Run `/vibe-cartographer-spec` first — I need your spec and PRD before we can plan the build."
...
- Append a `## /vibe-cartographer-checklist` section to `process-notes.md`.
- **Friction triggers contract:** [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-checklist`. The vibe-cartographer-friction-logger invocations below implement exactly the table there. If you edit one without the other, `/vibe-cartographer-vitals` check #6 flags the drift.
...
At workflow start, call `vibe-cartographer-session-logger.start("checklist", <project_dir>)` to get the sessionUUID. Hold it in memory for the duration of this workflow. Pass it to every `vibe-cartographer-friction-logger.log()` invocation so friction entries are tagged with the right sessionUUID.

At workflow end (after `docs/checklist.md` is generated and the `## /vibe-cartographer-checklist` section of `process-notes.md` is populated), call the vibe-cartographer-session-logger terminal-append procedure with the outcome and this same sessionUUID. Include `friction_notes`, `key_decisions`, `artifact_generated: "docs/checklist.md"`, and `complements_invoked` as applicable.

## Friction Logging

Reference: [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-checklist`. Invoke `vibe-cartographer-friction-logger.log()` at exactly these triggers, with exactly these confidence levels:

- **User reorders >25% of the generated checklist items before starting `/vibe-cartographer-build`** → `friction_type: "sequence_revised"`, `confidence: "high"`. Concrete signal — the agent's dependency analysis was off.
...
- **User rewrites >50% of generated `checklist.md`** (item descriptions, not order — order changes are `sequence_revised`) → `friction_type: "artifact_rewritten"`, `confidence: "medium"`. Distinguish content rewrite from order rewrite. Measured at `/vibe-cartographer-reflect` time; log call references this run's sessionUUID.
...
This follows the two-phase deepening rounds pattern in the `vibe-cartographer-guide` skill, though the checklist is more procedural than the earlier planning commands. The heavy thinking happened in /vibe-cartographer-spec — this is translation into an ordered, executable plan.
...
After the mandatory questions and initial checklist draft, offer the choice (see the `vibe-cartographer-guide` skill's Deepening-round mechanics for the pattern).
...
- **Builder mode:** Default to skipping. "Looks tight — should be good. Want a round of refinement, or move on?" Lead with moving on, not with the round. Builder-mode users have consistently chosen zero rounds on `/vibe-cartographer-checklist` (different from `/vibe-cartographer-prd` and `/vibe-cartographer-spec`, where they invest in deepening).

Good deepening questions for /vibe-cartographer-checklist include:
...
Quick — don't belabor these. One question at a time. The choices get encoded in the checklist header so `/vibe-cartographer-build` doesn't re-ask.
...
Whatever they choose, respect it. No mode switching mid-build — the choice is locked in once `/vibe-cartographer-build` starts.
...
- [ ] **N. [Clear title describing what's done when complete]**
  ...
  What to build: Concrete description of the work. Specific enough that /vibe-cartographer-build can execute without guessing.
...
This format is a contract with /vibe-cartographer-build — every item MUST have all five fields (title, spec ref, what to build, acceptance, verify). /vibe-cartographer-build reads each item and needs all five to execute properly.

Each item should be atomic — small enough to complete in one `/vibe-cartographer-build` session. If an item feels too big, break it down.
...
Read the template at `skills/vibe-cartographer-guide/templates/checklist-template.md`. Fill it in using everything from the conversation.
...
- Items are atomic — completable in one `/vibe-cartographer-build` session
...
- **If autonomous mode:** "Run `/vibe-cartographer-build` when you're ready. I'll work through the whole checklist and pause at checkpoints for you to verify." *(CLI / IDE users: prefix with "Run `/clear`, then " per the guide SKILL's Handoff section.)*
- **If step-by-step mode:** "Run `/vibe-cartographer-build` when you're ready — you'll run it once per checklist item. Each run picks up the next unchecked step." *(CLI / IDE users: run `/clear` between each item to fight context rot. Cowork users don't need to — context is managed automatically.)*

### Process Notes

Append to `process-notes.md` under the `## /vibe-cartographer-checklist` section:
...
- **This should be shorter than /vibe-cartographer-spec.** The heavy thinking is done. This is translation into an ordered plan. Don't belabor it.
...
- **The checklist is a living contract with /vibe-cartographer-build.** Whatever you write here is what the build agent will execute. The five-field format must be consistent across every item so /vibe-cartographer-build always knows where to find the spec ref, the work description, the acceptance criteria, and the verification step. But the checklist isn't sacred — if something breaks during the build, the checklist can and should be updated. That's normal. Plans meet reality and adapt.
