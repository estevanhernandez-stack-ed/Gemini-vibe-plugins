...
# /vibe-cartographer-spec — Blueprint Your App

Coach persona, posture, and the composition posture are always-on via `AGENTS.md`. The architecture-choice patterns (`references/spec-patterns.md`), the spec template, and the anchored-complements table (defer to `superpowers:writing-plans` / the Figma MCP if present) are in the `vibe-cartographer-guide` skill. Then follow this workflow.

**If the builder provided architecture docs during `/vibe-cartographer-onboard`**, read those now. They take precedence over `.agent/architecture/default-patterns.md` for stack choices, patterns, and conventions. If no architecture docs were provided, read `.agent/architecture/default-patterns.md` as your reference.
...
Read `shared.preferences.persona` from `~/.gemini/profiles/builder.json`. Your voice throughout this entire workflow — how you propose architecture, explain tradeoffs, and react to the builder's choices — must reflect the builder's chosen persona. See the `vibe-cartographer-guide` skill's Persona reference for the full table. Key behaviors per persona during /vibe-cartographer-spec:
...
`docs/scope.md` and `docs/prd.md` must exist. If either is missing: "Run `/vibe-cartographer-scope` and `/vibe-cartographer-prd` first — I need both before we can design the architecture."
...
- Append a `## /vibe-cartographer-spec` section to `process-notes.md`.
- **Friction triggers contract:** [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-spec`. The vibe-cartographer-friction-logger invocations below implement exactly the table there. If you edit one without the other, `/vibe-cartographer-vitals` check #6 flags the drift.
...
At workflow start, call `vibe-cartographer-session-logger.start("spec", <project_dir>)` to get the sessionUUID. Hold it in memory for the duration of this workflow. Pass it to every `vibe-cartographer-friction-logger.log()` invocation so friction entries are tagged with the right sessionUUID.

At workflow end (after `docs/spec.md` is generated and the `## /vibe-cartographer-spec` section of `process-notes.md` is populated), call the vibe-cartographer-session-logger terminal-append procedure with the outcome and this same sessionUUID. Include `friction_notes`, `key_decisions`, `artifact_generated: "docs/spec.md"`, and `complements_invoked` as applicable.

## Friction Logging

Reference: [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-spec`. Invoke `vibe-cartographer-friction-logger.log()` at exactly these triggers, with exactly these confidence levels:
...
- **User rewrites >50% of generated `spec.md`, especially the Stack or Component sections** (section-level diff is fine — doesn't need to be whole-file) → `friction_type: "artifact_rewritten"`, `confidence: "high"`. Measured at `/vibe-cartographer-reflect` time; log call references this run's sessionUUID.
...
This follows the two-phase deepening rounds pattern in the `vibe-cartographer-guide` skill. Your questions should be short and focused — draw the detail out of the builder. They should be doing most of the thinking and talking.
...
**2. Deployment — Identity & Signing.** Read `plugins.vibe-cartographer.deployment_target` from the unified profile (set during `/vibe-cartographer-onboard` step 11b). Three modes:
...
- **Target is named (GitHub release / Microsoft Store / npm / PyPI / Docker Hub / Cloudflare Pages / Vercel / other):** drill into the target's identity / signing / secret contract. Use the lookup below. These fields land in `docs/spec.md` under a new `## Deployment — Identity & Signing` heading that `/vibe-cartographer-checklist` consumes for release-related items.
- **Pattern #13 handoff (GitHub releases + `superpowers:vibe-launch` installed):** *"Your `vibe-launch` plugin can drive the release details — should we let it take over this section?"* Builder accepts → capture only target name in spec and yield to vibe-launch at `/vibe-cartographer-build` time; declines → gather the full GitHub row below.
...
**Rule:** only expand the one row matching the builder's target. Not every project ships — `/vibe-cartographer-spec` stays lean.
...
**5. File structure and data flow.** Build the full file tree together — every file, every folder, annotated with purpose. Walk through the lifecycle of the most important data in the app. Diagram it. These are the structural backbone that /vibe-cartographer-build relies on.

### Phase 2 — Deepening Rounds

After the mandatory questions, offer the choice (see the `vibe-cartographer-guide` skill's Deepening-round mechanics for the pattern).
...
Good deepening questions for /vibe-cartographer-spec include — calibrate depth to experience level from `builder-profile.md`:
...
- **Architecture self-review:** Step back and check your own work. Use subagents if available. Check for: ambiguities /vibe-cartographer-build would stumble on, PRD stories without a clear home in the architecture, complexity that doesn't fit the project scope, consistency between data model, file structure, and components. Surface 2-3 findings as genuine questions for the builder.
- **Subsystem config pattern-match (when extending existing infra):** When the spec adds new code to an existing subsystem with established config conventions — Cloud Functions (memory, timeout, region), MCP server tools (auth scope, schema shape), Firestore writers (transaction patterns, version fields), .NET csproj chains (per `Build-config hygiene` in `/vibe-cartographer-build`), Rust workspace profiles, etc. — the agent grepping existing values and using established floors as defaults is a load-bearing discipline. The vibe coder doesn't always have stack-specific mastery (Firebase Functions memory floors, Eventarc region constraints, Octokit auth wiring), and shouldn't have to. Architecture self-review explicitly checks: *"For every new file added to an existing subsystem, did I grep at least one sibling file's config values and use them as the floor?"* If not, surface as a finding — the builder may want to override, but the default should be "match the established pattern, then deviate intentionally."
...
When the builder chooses to proceed, read the template at `skills/vibe-cartographer-guide/templates/spec-template.md`. Fill it in using everything from the conversation.

**Critical requirements:**
- Every architectural component must have its own heading and subheadings — these become addresses for /vibe-cartographer-checklist
...
- Include URLs to reference docs the agent will need during /vibe-cartographer-build
...
"That spec is the heart of vibe direction — everything from here flows from it. Run `/vibe-cartographer-checklist` when you're ready." *(Claude Code CLI / VS Code / JetBrains users: prefix with "Run `/clear`, then " per the guide SKILL's Handoff section.)*

### Process Notes

Append to `process-notes.md` under the `## /vibe-cartographer-spec` section:
- Technical decisions made and rationale
- What the builder was confident about vs uncertain
- Stack choices and why
- **Deepening rounds:** How many rounds did the builder choose? What surfaced? Did deeper specification catch architecture issues that would have caused problems in /vibe-cartographer-build?
