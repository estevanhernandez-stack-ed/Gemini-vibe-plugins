...
# /vibe-cartographer-scope — Discover Your Project

Coach persona, posture, interaction rules, and the composition posture are always-on via `AGENTS.md`. The deepening-round mechanics and the anchored-complements table (defer the brain dump to `superpowers:brainstorming` if present) are in the `vibe-cartographer-guide` skill. Then follow this workflow.
...
`docs/builder-profile.md` must exist. If not: "Run `/vibe-cartographer-onboard` first — I need to know a bit about you before we dive in."
...
- Create `process-notes.md` in the project root if it doesn't exist. Add a header: `# Process Notes` and a section: `## /vibe-cartographer-scope`.
- **Friction triggers contract:** [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-scope`. The vibe-cartographer-friction-logger invocations below implement exactly the table there. If you edit one without the other, `/vibe-cartographer-vitals` check #6 flags the drift.
...
At workflow start, call `vibe-cartographer-session-logger.start("scope", <project_dir>)` to get the sessionUUID. Hold it in memory for the duration of this workflow. Pass it to every `vibe-cartographer-friction-logger.log()` invocation so friction entries are tagged with the right sessionUUID.

At workflow end (after `docs/scope.md` is generated and the `## /vibe-cartographer-scope` section of `process-notes.md` is populated), call the vibe-cartographer-session-logger terminal-append procedure with the outcome and this same sessionUUID. Include `friction_notes`, `key_decisions`, `artifact_generated: "docs/scope.md"`, and `complements_invoked` as applicable.

## Friction Logging

Reference: [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-scope`. Invoke `vibe-cartographer-friction-logger.log()` at exactly these triggers, with exactly these confidence levels:
...
- **User rewrites >50% of the generated `scope.md` post-write** (line diff between agent-generated and committed version) → `friction_type: "artifact_rewritten"`, `confidence: "high"`. Measure at `/vibe-cartographer-reflect` time, not in-line — the log call for this trigger happens from `/vibe-cartographer-reflect` with the sessionUUID of this `/vibe-cartographer-scope` run referenced via `friction_entry_ref` discipline.
- **User skips ahead to `/vibe-cartographer-prd` or `/vibe-cartographer-spec` without `/vibe-cartographer-scope` finishing** (detected by a next-command sentinel from the same project within 1h with no `/vibe-cartographer-scope` terminal) → `friction_type: "sequence_revised"`, `confidence: "medium"`.
...
Read `shared.preferences.persona` from `~/.gemini/profiles/builder.json`. Your voice throughout this entire workflow — how you interview, react, explain, and give feedback — must reflect the builder's chosen persona. See the `vibe-cartographer-guide` skill's Persona reference for the full table. Key behaviors per persona during /vibe-cartographer-scope:
...
This follows the two-phase deepening rounds pattern in the `vibe-cartographer-guide` skill. The core interview is a set of mandatory questions, followed by optional deepening rounds where the builder can keep refining before you generate the document.
...
These are the bare minimum to produce a meaningful scope doc. But unlike the other planning commands, /vibe-cartographer-scope is where you need to pull the most raw material out of the builder. Don't feel canned. Be adaptive. The goal is to get them talking — a lot — so that everything downstream has rich context to work with.
...
Good deepening questions for /vibe-cartographer-scope include:
...
When the builder chooses to proceed, read the template at `skills/vibe-cartographer-guide/templates/scope-template.md`. Fill it in using everything from the conversation. The scope doc should feel like a distillation of the conversation, not a form you filled out.
...
"Run `/vibe-cartographer-prd` when you're ready." *(Claude Code CLI / VS Code / JetBrains users: prefix with "Run `/clear`, then " per the guide SKILL's Handoff section.)*

### Process Notes

Append to `process-notes.md` under the `## /vibe-cartographer-scope` section:
