...
# /vibe-cartographer-reflect — Retro and Review

Read `skills/vibe-cartographer-guide/SKILL.md` for your overall behavior, then read `skills/vibe-cartographer-guide/references/eval-rubric.md` for the review dimensions. Follow this command.
...
Read `shared.preferences.persona` from `~/.gemini/profiles/builder.json`. Your voice throughout this entire workflow — how you run the check-in, deliver the review, and close — must reflect the builder's chosen persona. See the `vibe-cartographer-guide` skill's Persona reference for the full table. Key behaviors per persona during /vibe-cartographer-reflect:
...
- **Friction triggers contract:** [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-reflect`. The vibe-cartographer-friction-logger invocations below implement exactly the table there. If you edit one without the other, `/vibe-cartographer-vitals` check #6 flags the drift.
...
- **Data contracts:** [`.agent/skills/vibe-cartographer-guide/references/data-contracts.md`](.agent/skills/vibe-cartographer-guide/references/data-contracts.md) — the "Friction log" and "Friction calibration" sections define the shapes read and written during the calibration check-in. `friction.jsonl` is the read source; `friction.calibration.jsonl` is the write target. Both use `node .agent/scripts/atomic-append-jsonl.js`.

## Session Logging

At workflow start — before reading `docs/` or running Part A — call `vibe-cartographer-session-logger.start("reflect", <project_dir>)` to get the sessionUUID. Hold it in memory for the duration of this workflow. Pass it to every `vibe-cartographer-friction-logger.log()` invocation so friction entries are tagged with the right sessionUUID.

At workflow end — after `docs/reflection.md` has been written, after the unified profile has been updated, and after the closing message — call the vibe-cartographer-session-logger terminal-append procedure (`end(entry)`) with the **same sessionUUID** returned by `start()`. Set `outcome: "completed"` if the full Part A + Part B + calibration + generation flow ran, `"partial"` if the builder opted out mid-flow but reflection.md still got written, `"abandoned"` only in the rare case the command exited without producing reflection.md. Populate `friction_notes`, `key_decisions` (e.g., "builder skipped calibration check-in", "marked 4 entries false_positive"), `artifact_generated: "docs/reflection.md"`, and `complements_invoked` (e.g., `superpowers:requesting-code-review`) from what actually happened.

## Friction Logging

Reference: [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-reflect`. Invoke `vibe-cartographer-friction-logger.log()` at exactly these triggers, with exactly these confidence levels:

- **User declines the calibration check-in** (`[skip]` to "want to mark false positives?") → `friction_type: "default_overridden"`, `confidence: "low"`. The check-in is itself optional — declines are expected. Confidence stays low so it doesn't pollute the calibration signal or trigger `/vibe-cartographer-evolve` noise.
...
- **User rewrites >50% of generated `reflection.md`** → `friction_type: "artifact_rewritten"`, `confidence: "medium"`. Reflections are personal — confidence stays medium. Measured at the *next* `/vibe-cartographer-reflect` or `/vibe-cartographer-evolve` read time, not in-line; the log call at that future read references this run's sessionUUID.
- **User marks 3+ entries `false_positive` during the calibration check-in** → **no friction entry.** Calibration is its own signal channel — `/vibe-cartographer-reflect` never logs friction-about-friction. Calibration entries go to `friction.calibration.jsonl`, not `friction.jsonl`.
...
3. **What the process caught vs missed.** "What did the workflow catch that you would have missed working solo? What did it miss that you wish it had caught?" — Honest both-sides. Pull specifics: friction notes the agent flagged, edge cases surfaced during deepening rounds, things `/vibe-cartographer-iterate` had to clean up that `/vibe-cartographer-build` shipped, etc.

4. **Hindsight on the workflow itself.** "If you ran another project tomorrow with this same workflow, what's one thing you'd do differently — about how YOU showed up, not what the plugin should change?" — The plugin-improvement reflection happens in `/vibe-cartographer-evolve`. This question is about the builder's own practice. The agent should resist the temptation to redirect to "what should the plugin do better."
...
**Cross-doc `artifact_rewritten` measurement** (runs silently during this review phase, before presenting feedback — implements the "measured at /vibe-cartographer-reflect time" rows from `friction-triggers.md` for /vibe-cartographer-scope, /vibe-cartographer-prd, /vibe-cartographer-spec, /vibe-cartographer-checklist):

For each of `docs/scope.md`, `docs/prd.md`, `docs/spec.md`, and `docs/checklist.md` that exists:

1. **Find the originating session.** Scan `~/.gemini/antigravity/data/vibe-cartographer/sessions/*.jsonl` for the terminal entry whose `artifact_generated` matches this file and `command` matches the corresponding command (scope → /vibe-cartographer-scope, prd → /vibe-cartographer-prd, etc.). If multiple runs touched the same file, use the most recent one before this `/vibe-cartographer-reflect`. Capture its `sessionUUID` and `timestamp` — those tag the forthcoming log call.
...
4. **If `diff_pct > 0.5`**, call `vibe-cartographer-friction-logger.log()` with:
   ...
   - `sessionUUID: <originating session's UUID>` (from step 1 — NOT this `/vibe-cartographer-reflect`'s sessionUUID)
   - `timestamp`: now (captured by `log()` — this records when the measurement fired, while `sessionUUID` preserves the attribution)
   - `confidence: "high"` for /vibe-cartographer-scope and /vibe-cartographer-prd; `"high"` for /vibe-cartographer-spec; `"medium"` for /vibe-cartographer-checklist (per the triggers table — /vibe-cartographer-checklist distinguishes content rewrite from order rewrite, and this measurement only catches content).
   - `symptom: "<diff_pct rounded to nearest 5%> rewrite of <file> between agent generation (<originating timestamp>) and /vibe-cartographer-reflect time"`
5. **Order rewrites on /vibe-cartographer-checklist specifically.** The /vibe-cartographer-checklist triggers table distinguishes `artifact_rewritten` (content rewrite) from `sequence_revised` (order rewrite). The line-diff above catches both together. If nearly all the diff is line reordering rather than content edits (you can tell because `git diff --stat` shows balanced +/- counts with similar line bodies), log as `sequence_revised` with `confidence: "high"` instead.
...
Look at `prd.md` and the process notes for the /vibe-cartographer-prd phase. Did real expansion happen from scope to PRD? Are acceptance criteria testable? Were edge cases surfaced? Check how many deepening rounds the builder chose — extra rounds usually produce sharper criteria and fewer surprises during the build. If they skipped rounds and the PRD has gaps, connect those dots. One thing that landed, one to tighten.

**3. Technical Decisions**
Look at `spec.md`, `checklist.md`, and the process notes for /vibe-cartographer-spec and /vibe-cartographer-checklist. Were stack choices intentional? Does the architecture trace back to requirements? Was the build sequence logical? Did extra specification depth prevent build problems, or did skipping it cause issues? One thing that landed, one to tighten.
...
This is the last step of Part B. It's a quick loop that lets the builder correct the plugin's friction detector — marking false positives so `/vibe-cartographer-evolve` doesn't act on noise, and describing false negatives the logger missed. Pattern #6 (Friction Log) pairs with this check-in: the logger captures conservatively during commands, and this loop recovers missed signal while scrubbing false positives. Data contract: [`.agent/skills/vibe-cartographer-guide/references/data-contracts.md`](.agent/skills/vibe-cartographer-guide/references/data-contracts.md) — "Friction calibration" section.

**Step 1 — Read this project's friction entries.**

Read `~/.gemini/antigravity/data/vibe-cartographer-friction.jsonl` line-by-line. Parse each as JSON; silently skip malformed lines (`/vibe-cartographer-vitals` check #8 owns repair). Filter to entries matching this project's session window:

- `project_dir` equals the current project_dir.
- AND EITHER:
  - The entry was written after the previous `/vibe-cartographer-reflect` terminal entry for this project_dir (look back through the last ~30 days of session files for a prior `/vibe-cartographer-reflect` terminal entry in this project; the timestamp of that entry is the window start).
  - OR, if no prior `/vibe-cartographer-reflect` terminal exists, the entry is within the last 14 days.
...
>   1. [04-10 14:22] /vibe-cartographer-build — declined superpowers:test-driven-development
>   2. [04-11 09:15] /vibe-cartographer-spec — declined claude-api
>   3. [04-14 16:40] /vibe-cartographer-iterate — declined simplify
>
> **artifact_rewritten (2)**
>   4. [04-12 11:00] /vibe-cartographer-scope — 65% rewrite of scope.md between agent generation and /vibe-cartographer-reflect time
>   5. [04-13 19:30] /vibe-cartographer-prd — 52% rewrite of prd.md between agent generation and /vibe-cartographer-reflect time
>
> **default_overridden (1)**
>   6. [04-10 13:50] /vibe-cartographer-build — chose autonomous when I recommended step-by-step
>
...
- **If the builder replies with `fp <number>` markings:** for each marked entry, append ONE line to `friction.calibration.jsonl` via `node .agent/scripts/atomic-append-jsonl.js ~/.gemini/antigravity/data/vibe-cartographer-friction.calibration.jsonl`. Schema:
  ```json
  {
    "schema_version": 1,
  ...
  ```
  Each mark is one append. Validate against `friction-calibration.schema.json` before writing; silent-drop on validation failure (same defensive default as `vibe-cartographer-friction-logger.log()`).
- **If the builder describes free-text friction you missed** (e.g., "I felt friction when you suggested X but I didn't push back"): append a `false_negative` calibration entry. Because there is no exact matching friction.jsonl line, anchor `friction_entry_ref` to the nearest relevant session entry — ideally the terminal entry of the command the builder is describing (its `(timestamp, friction_type: <best-fit enum>, sessionUUID)` triple). If no clean anchor exists, use this `/vibe-cartographer-reflect`'s sessionUUID and the entry's timestamp, with `friction_type` picked from the seven canonical types based on what the builder described. Put the builder's description in `builder_note`.
...
After writes complete, one short line: "Logged N marks — that'll feed into `/vibe-cartographer-evolve` next time you run it. Let's pull together your reflection."

If the builder declined (`skip`), that itself is a `default_overridden` friction entry per the /vibe-cartographer-reflect trigger table — log it now via `vibe-cartographer-friction-logger.log()` with `friction_type: "default_overridden"`, `confidence: "low"`, and pass this `/vibe-cartographer-reflect`'s sessionUUID. Confidence stays low because declining the check-in is expected behavior — this is a weak signal, not a complaint about the check-in itself.

**Defensive behaviors:**

- The calibration interface NEVER writes to `friction.jsonl` — only to `friction.calibration.jsonl`. These are distinct streams. `/vibe-cartographer-reflect` never logs friction-about-friction (per the /vibe-cartographer-reflect triggers table).
...
Read the template at `skills/vibe-cartographer-guide/templates/reflection-template.md`. Fill it in using the observations and reflection.
...
After generating the reflection, update the **unified cross-plugin profile** at `~/.gemini/profiles/builder.json`. This is the same file the onboard SKILL writes to — shared across all 626Labs plugins.

**Read-merge-write procedure:**

1. Read `~/.gemini/profiles/builder.json` if it exists. If it doesn't exist (builder skipped onboard or used v0.4.0), create it now using the schema defined in the onboard SKILL.
...
7. Log the update in `process-notes.md` under the `/vibe-cartographer-reflect` section: "Updated unified builder profile — [what changed]."
