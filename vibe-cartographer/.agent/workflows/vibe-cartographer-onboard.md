...
# /vibe-cartographer-onboard — Welcome and Meet the Builder

Coach persona, posture, interaction rules, the unified-profile ownership contract, and the decay wiring are always-on via `AGENTS.md`. The per-persona behavior table and mode dimension tables (you set both here) are in the `vibe-cartographer-guide` skill; the profile schema is in `vibe-cartographer-guide/schemas/builder-profile.schema.json` and the stale-field re-validation in the `vibe-cartographer-decay` skill. Then follow this workflow.
...
**Run this bash command** (ignore any errors — this is best-effort, offline/network issues must NOT block /vibe-cartographer-onboard):

```bash
INSTALLED=$(cat ~/.npm-global/lib/node_modules/@esthernandez/vibe-cartographer/plugins/vibe-cartographer/.agent/agent.json 2>/dev/null \
  || find "$APPDATA/npm/node_modules/@esthernandez/vibe-cartographer/plugins/vibe-cartographer/.agent/agent.json" 2>/dev/null \
...
**Rules:**
- Run this ONCE at the very start of /vibe-cartographer-onboard, before any other output
- If the command fails, errors, times out, or returns nothing — silently continue with the rest of /vibe-cartographer-onboard
...
- **Read everything in `docs/` first.** Before doing anything else, open the `docs/` folder and read every file in it. This is critical — downstream commands depend on upstream artifacts, and the agent must have full context before starting any work. For /vibe-cartographer-onboard this folder will usually be empty, but always check.
- Create `process-notes.md` in the project root if it doesn't exist. Add a header: `# Process Notes` and a section: `## /vibe-cartographer-onboard`.
- **Friction triggers contract:** [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-onboard`. The vibe-cartographer-friction-logger invocations below implement exactly the table there. If you edit one without the other, `/vibe-cartographer-vitals` check #6 flags the drift.
...
At workflow start, call `vibe-cartographer-session-logger.start("onboard", <project_dir>)` to get the sessionUUID. Hold it in memory for the duration of this workflow. Pass it to every `vibe-cartographer-friction-logger.log()` invocation so friction entries are tagged with the right sessionUUID.

At workflow end (after `docs/builder-profile.md` is written, the unified profile is updated, and the `## /vibe-cartographer-onboard` section of `process-notes.md` is populated), call the vibe-cartographer-session-logger terminal-append procedure with the outcome and this same sessionUUID. Include `friction_notes`, `key_decisions`, `artifact_generated: "docs/builder-profile.md"`, and `complements_invoked` as applicable.

**Order at workflow start (once all pieces are wired): version check → vibe-cartographer-session-logger.start() → decay check → rest of onboard.** The vibe-cartographer-session-logger sentinel must be on disk before the decay check runs so `vibe-cartographer-friction-logger.detect_orphans()` can pair a sentinel to a terminal for this run.

Also invoke `vibe-cartographer-friction-logger.detect_orphans()` once at startup (after vibe-cartographer-session-logger.start() writes the sentinel, before the decay check). Any sentinels from prior runs older than 24h without a matching terminal get emitted as `command_abandoned` friction entries. This is the out-of-band orphan sweep — do not attempt to also log `command_abandoned` from inside the regular friction trigger table.

## Friction Logging

Reference: [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-onboard`. Invoke `vibe-cartographer-friction-logger.log()` at exactly these triggers, with exactly these confidence levels:
...
- **User abandons mid-onboarding, picks back up later, but skips the resumed prompts and re-runs `/vibe-cartographer-onboard` from scratch** (detected via sentinel-without-terminal followed by a fresh sentinel for the same project within 24h) → `friction_type: "sequence_revised"`, `confidence: "medium"`. Surface the pattern in `symptom`.
...
Every `log()` call passes the sessionUUID returned by `vibe-cartographer-session-logger.start()` at the top of this command so entries cluster under this run.

## Unified Builder Profile Check

Before starting the flow, check the **unified builder profile** at `~/.gemini/profiles/builder.json`. This is the shared cross-plugin profile — a single source of truth for builder identity and preferences that any 626Labs plugin can read.

**Check order:**

1. Read `~/.gemini/profiles/builder.json` if it exists.
...
3. If the unified profile doesn't exist at all, check the **deep legacy location** `~/.gemini/antigravity/data/app-project-readiness/user-profile.md` (from v0.4.0 and earlier). If present, migrate it directly into `plugins.vibe-cartographer` and rename the markdown file to `.bak`.
...
### Decay Check (Pattern #4 — runs at workflow start)

After the profile is loaded and after the vibe-cartographer-session-logger sentinel entry is written (see Session Logging above — `start()` must fire first so the sentinel is on disk before decay runs), invoke the internal **decay** SKILL at `skills/vibe-cartographer-decay/SKILL.md`. It implements Pattern #4 (Memory Decay and Refresh) from `docs/self-evolving-plugins-framework.md`.

**Two procedures, in this order:**

1. **Fresh-stamp migration (1.4.x → 1.5.0).** If the loaded profile has no `shared._meta` block, run the silent migration described in `skills/vibe-cartographer-decay/SKILL.md`: stamp every shared decay-eligible field that's actually present in the profile with today's date and the default TTL. No user prompt, no console output. Atomic-write the profile back. If the write fails, log the error in `process-notes.md` and skip the decay check for this run.
...
- After the user responds:
  ...
  - If they update the value — write the new value into the profile **first**, then call `stamp(field_path)`. The stamp does not modify field values; only `/vibe-cartographer-onboard` writes the new value.
...
The file at `~/.gemini/profiles/builder.json` has this shape:
...
**Legacy namespace (v0.5.0 → v1.0.0 rename):** if `~/.gemini/profiles/builder.json` exists and contains a `plugins.vibe-cartographer` block, copy it to `plugins.vibe-cartographer`, keep the old key in place for one release as a safety net, bump `last_updated`, and note the migration in `process-notes.md`: "Migrated `plugins.vibe-cartographer` → `plugins.vibe-cartographer` on builder profile."

**Deep legacy path (v0.4.0 and earlier):** if neither the unified profile nor the legacy namespace exists, check `~/.gemini/antigravity/data/app-project-readiness/user-profile.md`. If present:
...
4. Create `~/.gemini/antigravity/profiles/` directory if it doesn't exist (`mkdir -p`).
5. Write the JSON file at `~/.gemini/profiles/builder.json`.
...
**Pacing confirmation (always — first behavioral beat of /vibe-cartographer-onboard, applies to both branches).** Right after the banner displays, before *anything* else, ask the builder whether to run this conversationally or autonomously. The default is conversational. `/vibe-cartographer-onboard` is built around back-and-forth interviews — running it without the builder in the chair defeats the entire flipped-interaction premise. Even if the harness is in autonomous / auto mode, do NOT take that as permission to invent answers. Pause and ask explicitly.

Phrasing (one question, free-form prose — no multiple-choice tools per the Conversation Style at the bottom of this SKILL):

> *"Quick check before we start — `/vibe-cartographer-onboard` is a back-and-forth interview, one question at a time. Want me to (a) run conversationally and pause for your answers (default), or (b) plow through with reasonable defaults pulled from any profile on file? Default is (a) — answer or just hit enter to keep going conversationally; say 'autonomous' or 'just go' if you actually want (b)."*
...
- **"(b)", "autonomous", "just go", "yolo", anything affirming (b)** → autonomous. **From this point on, the run flows end-to-end without further pauses.** Once the builder has opted in at this single gate, do NOT ask any more questions for the rest of `/vibe-cartographer-onboard` — no per-step confirmations, no "still good?" check-ins, no embedded decay confirmations (any stale `_meta` field is deferred and surfaces again on the next interactive run; do not stamp it). Skip the entire interview (steps 3 through 11b) and go straight to artifact generation (steps 12 and 13). Use this fill-strategy:
  - **Returning-builder branch:** pull every value from the unified profile (`~/.gemini/profiles/builder.json`). For any field that's missing or marked stale, use the defaults table below.
  - **New-builder branch:** the profile is mostly empty. Apply these defaults — `name: "Builder"` (placeholder), `technical_experience.level: "intermediate"`, `technical_experience.languages: []`, `technical_experience.frameworks: []`, `mode: "builder"`, `persona: null` (system default), `tone: "casual, direct"`, `pacing: "brisk"`, `creative_sensibility: "Clean and functional. Refine in /vibe-cartographer-scope or /vibe-cartographer-reflect."`, `project_origin: "blank folder (assumed)"`, `project_goals: "to be confirmed in /vibe-cartographer-scope"`, `design_direction: "no strong signals — default to clean and functional"`, `deployment_target: null`. The builder-profile-portability hint still emits (information-only, no pause).
  - **Both branches:** generate `docs/builder-profile.md` and update the unified profile per steps 12-13. Surface every value used (whether pulled from profile or defaulted) in `docs/builder-profile.md` *and* in a new `## /vibe-cartographer-onboard — autonomous run` section of `process-notes.md`, marking each defaulted value with `(default — confirm on next run)`. Then hand off to `/vibe-cartographer-scope` per the existing Handoff section.

  **The contract is: opt in once at the pacing question, then Cart gets out of the way and produces artifacts. Visibility is the contract — every assumption is recorded — but not pausing is also the contract. Don't ask for confirmation. Flow.** Builders who want to refine just re-run `/vibe-cartographer-onboard` interactive on the next pass.
- **Ambiguous answer** → ask once more, terser. After that, fall back to conversational (the safe default).

Ask this BEFORE branching into returning-builder vs new-builder. Do not ask twice in the same run. The check applies to every `/vibe-cartographer-onboard` invocation regardless of whether a profile already exists.
...
Then name the full command chain: `/vibe-cartographer-onboard` (that's now) → `/vibe-cartographer-scope` → `/vibe-cartographer-prd` → `/vibe-cartographer-spec` → `/vibe-cartographer-checklist` → `/vibe-cartographer-build` → `/vibe-cartographer-iterate` (optional) → `/vibe-cartographer-reflect`.
...
This can be anything: shipping to users, building a portfolio piece, solving a personal problem, learning a new stack, validating an idea. Their answer gets captured in the builder profile and `/vibe-cartographer-reflect` loops back to it at the end to see how they did.
...
- **Greenfield** (blank folder, starting fresh) → full `/vibe-cartographer-scope → /vibe-cartographer-prd → /vibe-cartographer-spec → /vibe-cartographer-checklist → /vibe-cartographer-build` flow works well. The scripted pace matches the empty canvas.
- **No-code escape** (Bolt/Lovable/v0/Replit Agent prototype) → `/vibe-cartographer-scope` and `/vibe-cartographer-prd` compress naturally because the user already has a working artifact to point at. `/vibe-cartographer-spec` and `/vibe-cartographer-build` focus on translating the prototype into maintainable code, not designing from scratch. Some deepening rounds may be redundant.
- **Extending existing repo** → `/vibe-cartographer-spec` pulls from the existing codebase's architecture, patterns, and conventions rather than treating it as greenfield. `/vibe-cartographer-build` integrates into the existing structure.
...
Keep this brief. Don't probe deeply. Take what they give you and move on. The goal is a signal about the app's design direction that can inform decisions in `/vibe-cartographer-scope` and `/vibe-cartographer-spec`. If they have nothing specific, that's fine — note "No strong signals — default to clean and functional" and move on.

### 8. Gauge Prior SDD Knowledge

Before wrapping up, get a lightweight read on whether the builder already has experience with structured development processes. This helps `/vibe-cartographer-reflect` calibrate its quiz — an experienced developer who already practices something like SDD should get different questions than someone encountering these ideas for the first time.
...
The agent should **commit to the persona** from this point forward — adopt its voice, checkpoint style, and explanation depth consistently. See the **Persona Adaptation** section in `skills/vibe-cartographer-guide/SKILL.md` for how each persona affects concrete behavior.
...
- Confirm: "Got it — I'll use these to guide the technical decisions in `/vibe-cartographer-spec` and beyond."

If they don't have architecture docs:
- Note "No architecture docs provided — will use defaults" in the builder profile
- That's fine. The `/vibe-cartographer-spec` step will fall back to `architecture/default-patterns.md` and work with the builder to choose a stack.
...
question) lets `/vibe-cartographer-spec` drill into the target's Identity & Signing fields later
...
- If the builder names a target, `/vibe-cartographer-spec` will surface a target-specific
...
- If "not sure yet" / "internal only" / null, `/vibe-cartographer-spec` skips the drill-down.

### 12. Generate `docs/builder-profile.md`

Read the template at `skills/vibe-cartographer-guide/templates/builder-profile-template.md`. Fill it in using everything from the conversation.
...
After writing the per-project builder profile, create or update the **unified cross-plugin profile** at `~/.gemini/profiles/builder.json`. This is about the **person**, not the project — it carries across all future projects AND all other 626Labs plugins.

**Write procedure:**

1. Ensure the directory exists: `mkdir -p ~/.gemini/antigravity/profiles/`.
...
- `projects_completed` — integer, only incremented by `/vibe-cartographer-reflect` at end of a full run
...
"Great — I've got everything I need. Run `/vibe-cartographer-scope` when you're ready — that's where we'll find your idea." *(If the builder is in Claude Code CLI / VS Code / JetBrains, prefix the handoff with "Run `/clear`, then " per the guide SKILL's Handoff section.)*

### Process Notes

Append to `process-notes.md` under the `## /vibe-cartographer-onboard` section:
