---
description: "Capture, refresh, or extend a coder voice profile in ~/.gemini/antigravity/AGENTS.md so the agent answers in a chosen voice during autonomous work. Always presents a choice — never imposes a path."
---

# /vibe-cartographer-coder-voice — Capture and Apply the Coder's Actual Voice

Persona, posture, hard rules, and voice are always-on via `AGENTS.md`. The cutter preset files and the synthesis-block conventions are situational — in the `vibe-cartographer-guide` skill (`.agent/skills/vibe-cartographer-guide/`). Then follow this workflow.

This workflow captures **the actual coder's voice** (the user's, not a synthesis of admired voices) and writes it to `~/.gemini/antigravity/AGENTS.md` so the agent answers in that voice during autonomous work. The file it WRITES is `AGENTS.md` — the live rules file every agent decision rests on. It's parallel to `vibe-thesis:voice` but inverted — the user is the base; admired coders are optional **Cross** modifiers, never replacements.

## Why this exists

When agents work autonomously over long sessions, output drifts toward generic-AI voice — corporate-speak, hedged framing, polite ceremony. A persisted voice profile in `~/.gemini/antigravity/AGENTS.md` (which loads on every Antigravity session) anchors the agent in the user's actual voice across every project, every repo, every working day.

This is **the coder's own voice**, not a stylized voice. For "pick a preset coder voice" use the `cutter` mode (Section 7) instead.

## Prerequisites

- Read access to `~/.gemini/antigravity/AGENTS.md` (the global persona file).
- Read access to the user's session transcripts under `~/.gemini/antigravity/projects/<project>/<session-uuid>.jsonl`.
...
This SKILL writes to `~/.gemini/antigravity/AGENTS.md`, which changes how the agent behaves in **every future session, every repo**. That's load-bearing global self-modification.
...
- **The harness may block the write** — `~/.gemini/antigravity/AGENTS.md` edits hit the same Self-Modification gate as `.claude/settings.json`. If blocked, offer the content for paste-in or suggest invoking via the `update-config` skill.
...
1. **`~/.gemini/antigravity/AGENTS.md`** — existing persona definition (The Architect, voice rules, tone, pace).
2. **`~/.gemini/profiles/builder.json`** — `shared.preferences.tone`, `pacing`, `communication_style`, `creative_sensibility`, plus any plugin-namespace voice notes.
3. **Session transcripts** at `~/.gemini/antigravity/projects/<project>/<session-uuid>.jsonl` — **first-class source**. Sample the most recent 5-10 sessions across diverse projects. Filter each line to `type: "user"` AND `message.content` is text (not tool results). Skip lines starting with `<` (system reminders), `Caveat:`, `[Pasted` (pasted blobs), or containing `system-reminder` / `tool_use_id` in the first 200 chars. Keep messages between 30 and 800 chars for voice signal.
4. **Project memories** at `~/.gemini/antigravity/projects/<project>/memory/*.md` — especially `user_*.md` files (e.g., `user_voice_lizzing.md`, `user_profile_context.md`). Read MEMORY.md index first.
...
**This is the default entry path.** When the user runs `/vibe-cartographer-coder-voice` and there's no existing `## CODER VOICE SYNTHESIS` block, do not pull them into discover-first automatically. Present a three-way choice. Voice profiles are an option, not a demand.
...
Voice profile not set. Three paths — pick what fits, none are required:
  ...
  [personal]  Capture YOUR actual voice. The skill sweeps your corpus
              (~/.gemini/antigravity/AGENTS.md, ~/.gemini/profiles/builder.json, recent
              session transcripts under ~/.gemini/antigravity/projects/, memories, blog
              posts, process-notes) and synthesizes a profile, or falls back
              to a 3-question interview if corpus is thin. Profile lives at
              ~/.gemini/antigravity/AGENTS.md and applies across every Antigravity session.
  ...
  [skip]      Don't set a voice profile. The default Architect persona keeps
              running. Run /vibe-cartographer-coder-voice anytime later to set one up.
...
- **`skip`** → exit cleanly: `"Got it. /vibe-cartographer-coder-voice anytime if you want to set one up later."` Don't write anything.

### 3. Always confirm before any write

Regardless of path, never write to `~/.gemini/antigravity/AGENTS.md` without explicit yes. Self-modification gate applies to all paths equally.
...
I'm reading: ~/.gemini/antigravity/AGENTS.md, ~/.gemini/profiles/builder.json,
...
and confirm before writing to ~/.gemini/antigravity/AGENTS.md.
...
`/vibe-cartographer-coder-voice cross` after the base lands).
```
...
> for "pick a preset coder voice" use `/vibe-cartographer-coder-voice cutter <preset>`.
```
...
```
[apply]  Write this block to ~/.gemini/antigravity/AGENTS.md after the existing
...
[now]    Run `/vibe-cartographer-coder-voice cross` immediately
...
Read `~/.gemini/antigravity/AGENTS.md`, locate `## CODER VOICE SYNTHESIS` → `### Cross — voice modifiers`. Three states:

- **No base block** → refuse politely. Tell the user to run `/vibe-cartographer-coder-voice` (no args) first.
...
Same as fresh mode — show the proposed Cross sub-block, confirm, write to `~/.gemini/antigravity/AGENTS.md` via `Edit`. If self-modification gate blocks, fall back to paste-in.

## Flow — revise mode

When a block already exists and the user runs `/vibe-cartographer-coder-voice` with no args, default to revise mode:
...
Read `.agent/skills/vibe-cartographer-guide/cutters/<preset>.md`. The file contains:
...
Print the full body content as a **proposed write**, NOT applied. Let the user see exactly what will land in `~/.gemini/antigravity/AGENTS.md`.

```
About to write the {preset} cutter to ~/.gemini/antigravity/AGENTS.md.
...
[apply]    Write to ~/.gemini/antigravity/AGENTS.md
...
1. Read `~/.gemini/antigravity/AGENTS.md`.
...
Cross layer is still available with /vibe-cartographer-coder-voice cross — you can mix
...
To add a new preset, drop a markdown file at `.agent/skills/vibe-cartographer-guide/cutters/<name>.md` with:
...
> "pick a preset coder voice" use `/vibe-cartographer-coder-voice cutter <preset>`.
...
Sections that don't apply (e.g., empty Cross when only base ran) are written as headings with a one-line "deferred — run `/vibe-cartographer-coder-voice cross` to populate" placeholder. Never silently omit sections.

## Edge cases

- **No `~/.gemini/antigravity/AGENTS.md` at all.** Refuse politely. Tell the user to run `/keystone` (the global keystone bootstrap) first, or create a minimal `~/.gemini/antigravity/AGENTS.md` themselves with at least a persona section.
- **Existing block was hand-edited by the user since last `/vibe-cartographer-coder-voice` run.** Detect via section structure mismatch. Don't clobber — show diff, ask "merge / replace / leave alone."
...
- **Cross mode requested but no base voice exists.** Refuse politely. Direct user to run `/vibe-cartographer-coder-voice` (no args) first to lay down the base.
...
This SKILL doesn't append a session log entry — it's a meta/setup SKILL, not part of the build chain. Skip the `vibe-cartographer-session-logger` start/end protocol that build-chain SKILLs use.
...
- **Never auto-write to `~/.gemini/antigravity/AGENTS.md` without explicit yes.** Self-modification needs per-run confirmation.
- **Never skip the session-transcript source.** It's the richest voice signal available; treating it as optional is the failure mode that birthed this SKILL (see /vibe-cartographer-coder-voice's own friction history).
...
- **Never hardcode the user's name, location, or identifying details into the block.** Voice rules are voice rules; PII stays in `~/.gemini/profiles/builder.json` and project-local memories.
- **Never share another user's voice profile across the SKILL boundary.** This SKILL writes to ONE user's `~/.gemini/antigravity/AGENTS.md`. It never reads or writes another user's profile.
...
No handoff to another command. `/vibe-cartographer-coder-voice` is a standalone setup SKILL. After the block lands, the agent reads it on every future session start (since `~/.gemini/antigravity/AGENTS.md` is loaded globally).

"Voice profile written. Next session you start, the agent reads this and adapts. Re-run `/vibe-cartographer-coder-voice` anytime to refresh, or `/vibe-cartographer-coder-voice cross` to add the modifier layer."
