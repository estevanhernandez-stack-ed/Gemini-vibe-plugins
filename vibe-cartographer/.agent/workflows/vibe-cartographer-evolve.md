...
# /vibe-cartographer-evolve — Reflective Evolution

Coach persona, posture, the self-evolving-framework wiring, and the "nothing auto-applies" rule are always-on via `AGENTS.md`. The data contracts, friction-trigger map, and the schemas you reason over when proposing edits are in the `vibe-cartographer-guide` skill; the session/friction logs you read are at `~/.gemini/antigravity/data/vibe-cartographer/`. Then follow this workflow.
...
- The unified builder profile at `~/.gemini/profiles/builder.json` must exist (builder has run `/vibe-cartographer-onboard` at least once).
- At least one session log entry must exist at `~/.gemini/antigravity/data/vibe-cartographer/sessions/*.jsonl`. If not: "You haven't run a full session yet. Run `/vibe-cartographer-onboard` → `/vibe-cartographer-scope` through `/vibe-cartographer-reflect` at least once, then come back."

## Before You Start

- Read every `.jsonl` file in `~/.gemini/antigravity/data/vibe-cartographer/sessions/`. Each line is a JSON entry per the vibe-cartographer-session-logger SKILL schema.
- Read the unified profile at `~/.gemini/profiles/builder.json` for baseline context (experience level, persona, preferences).
- Read the plugin's own workflow/skill files so you can propose specific, accurate diffs. **Self-edit target map (port layout — NOT the source `plugins/vibe-cartographer/skills/<cmd>/SKILL.md` paths):**
  - **Chain + tool command behavior** (onboard, scope, prd, spec, checklist, build, iterate, reflect, vitals, friction, coder-voice) → `.agent/workflows/vibe-cartographer-<cmd>.md` (these flipped from skills to workflows in the port).
  - **`evolve-cart` itself** → `.agent/workflows/vibe-cartographer-evolve.md`.
  - **Always-on behavior** (persona model, posture, hard rules, voice, composition posture) → `AGENTS.md` (folded out of the source guide).
  - **Situational reference** (per-persona table, mode tables, deepening mechanics, anchored-complements table, data contracts, friction-trigger map, schemas, templates, cutters) → `.agent/skills/vibe-cartographer-guide/` (SKILL.md + `references/` + `schemas/` + `templates/` + `cutters/`).
  - **Internal skills** (decay, session-logger, friction-logger) → `.agent/skills/vibe-cartographer-<skill>/SKILL.md`.
  - **Atomic-write helpers** → `.agent/scripts/atomic-*.js`. **Architecture defaults** → `.agent/architecture/`.
  - Never propose changes to the source-tree `plugins/...` paths — they don't exist in the port.
...
- **Friction triggers contract:** [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-evolve`. The vibe-cartographer-friction-logger invocations below implement exactly the table there. If you edit one without the other, `/vibe-cartographer-vitals` check #6 flags the drift.
...
- **Data contracts:** [`.agent/skills/vibe-cartographer-guide/references/data-contracts.md`](.agent/skills/vibe-cartographer-guide/references/data-contracts.md) — the "Friction log" and "Friction calibration" sections define the shapes of the two first-class inputs the Analyze phase now reads. `friction.jsonl` and `friction.calibration.jsonl` are both append-only JSONL streams under `~/.gemini/antigravity/data/vibe-cartographer/`; read line-by-line, silent-drop malformed lines (`/vibe-cartographer-vitals` check #8 owns repair).

## Session Logging

At workflow start — before reading session logs, profile, or friction files — call `vibe-cartographer-session-logger.start("evolve-cart", <project_dir>)` to get the sessionUUID. Hold it in memory for the duration of this workflow. Pass it to every `vibe-cartographer-friction-logger.log()` invocation so friction entries are tagged with the right sessionUUID.

At workflow end — after all proposals have been processed (applied / rejected / deferred) and the summary has been shown — call the vibe-cartographer-session-logger terminal-append procedure (`end(entry)`) with the **same sessionUUID** returned by `start()`. Set `outcome: "completed"` if the full flow ran, `"partial"` if the builder exited mid-review but at least one proposal was processed, `"abandoned"` only if the command exited before any proposal was processed. Populate `friction_notes`, `key_decisions` (e.g., "applied 2 Plugin-track, 1 Personal-track", "rejected complement-rejection proposal for superpowers:tdd"), `artifact_generated: null` (evolve-cart writes to SKILL files and the profile, not a single artifact), and `complements_invoked` from what actually happened. This terminal entry is **in addition to** the legacy `/vibe-cartographer-evolve` run-log entry described in step 7 below — both are written, with the same sessionUUID.

## Friction Logging

Reference: [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-evolve`. Invoke `vibe-cartographer-friction-logger.log()` at exactly these triggers, with exactly these confidence levels:

- **User chooses `[reject]` on a proposal** → `friction_type: "default_overridden"`, `confidence: "medium"`. Capture the proposal title in `symptom`. The fact that `/vibe-cartographer-evolve` itself proposed the change is implicit — no `complement_involved`.
...
```
Pulling session history from ~/.gemini/antigravity/data/vibe-cartographer/sessions/
...
1. **Read** `~/.gemini/antigravity/data/vibe-cartographer-friction.jsonl` line-by-line. Parse each as JSON; silently skip malformed lines (`/vibe-cartographer-vitals` check #8 owns repair). Each entry is a `friction.schema.json`-shaped record with `timestamp`, `command`, `friction_type`, `confidence`, `symptom`, `complement_involved` (sometimes null), `sessionUUID`, `project_dir`.
2. **Read** `~/.gemini/antigravity/data/vibe-cartographer-friction.calibration.jsonl` line-by-line. Same defensive parse. Each entry is a `friction-calibration.schema.json`-shaped record with `timestamp`, `friction_entry_ref: {timestamp, friction_type, sessionUUID}`, and `calibration: "false_positive" | "false_negative"`.
...
Calibration entries themselves age out. This prevents stale false-positive marks from permanently blinding `/vibe-cartographer-evolve` to shifted habits — a builder who marked "declined TDD" as a false positive 9 months ago may have genuinely shifted away from TDD in the interim.
...
After weighting, do a targeted pass for the most important signal `/vibe-cartographer-evolve` 1.5.0 gains: complements that the builder keeps rejecting. The Pattern #13 anchored-complements table in the `vibe-cartographer-guide` skill offers specific complements at specific trigger points — when a complement is rejected enough, that's evidence the anchored offer is wrong (either for everyone or for this builder).
...
- **Plugin track** — if the complement is anchored for **multiple commands** in the `vibe-cartographer-guide` skill's Pattern #13 anchored-complements table AND the rejection pattern spans multiple commands (e.g., the builder declines `superpowers:tdd` on `/vibe-cartographer-build` AND on `/vibe-cartographer-iterate`), this is a structural issue with the anchored table itself. Propose removing (or softening from "anchored" to "surfaceable") the complement for the affected commands. Ships to every user on next release.
...
| **Personal** | Preference specific to this builder's style | Write to `plugins.vibe-cartographer` block in `~/.gemini/profiles/builder.json` | Only this builder's future sessions |
| **Community (opt-in)** | Potentially useful signal but not confidently universal — *might* become a Plugin-track change later with more data | Appended to `~/.gemini/antigravity/data/vibe-cartographer/community-signals.jsonl` on this machine only | No one, until the builder explicitly exports and shares it |
...
- **Local-first:** all community signals are appended to `~/.gemini/antigravity/data/vibe-cartographer/community-signals.jsonl` on this machine, never transmitted.
...
1. "What would you reframe in this read? Is the pattern accurate, partial-but-narrow, or off entirely?" — first-pass observations are starting points, not landings. Across multiple `/vibe-cartographer-evolve` runs, builders consistently refine rather than accept-or-reject; lead with that posture as the default.
...
```
Proposal (Plugin track): Flip /vibe-cartographer-prd deepening rounds default for Builder mode.

Currently in .agent/workflows/vibe-cartographer-prd.md:
...
[apply]    Apply this change to .agent/workflows/vibe-cartographer-prd.md
...
Propose a write to the `plugins.vibe-cartographer` block in `~/.gemini/profiles/builder.json`. Show the exact JSON patch.
...
Propose appending an anonymized signal to `~/.gemini/antigravity/data/vibe-cartographer/community-signals.jsonl`. Show the exact entry before writing. **Require explicit yes every time — never assume opt-in.**
...
- Ensure `~/.gemini/antigravity/data/vibe-cartographer/community-signals.jsonl` exists (create the file if it doesn't).
...
Deferred: J proposals (saved for next /vibe-cartographer-evolve run).

Plugin files changed:
  • .agent/workflows/vibe-cartographer-onboard.md (Starting Point question)

Personal profile updates:
  • plugins.vibe-cartographer.prefers_skip_deepening = true

Community signals logged (local only, not sent):
  • ~/.gemini/antigravity/data/vibe-cartographer/community-signals.jsonl
...
Append a special session log entry for this `/vibe-cartographer-evolve` invocation to `~/.gemini/antigravity/data/vibe-cartographer/sessions/<date>.jsonl`:
...
```json
{
  ...
  "applied_files": [".agent/workflows/vibe-cartographer-prd.md", "..."],
...
- **Never delete calibration entries from disk as part of decay.** The 180-day TTL is a read-time filter only — the file is append-only history. `/vibe-cartographer-vitals` check #8 owns any structural repair of the calibration file; `/vibe-cartographer-evolve` never edits it.
...
No handoff to another command. `/vibe-cartographer-evolve` is a standalone reflection run. The builder commits the changes when they're ready.

"Thanks for reviewing. Whenever new patterns emerge, run `/vibe-cartographer-evolve` again."
