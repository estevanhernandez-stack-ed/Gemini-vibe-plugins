...
# /vibe-cartographer-vitals — Self-Diagnostic
...
This is Pattern #8 (Plugin Self-Test) from `docs/self-evolving-plugins-framework.md`, complete: diagnostic half (eight read-only checks) + remedial half (six per-fix interactive auto-fix prompts). **No fix ever runs without an explicit `[y/n]`.** Read-only checks never write; writes happen only in the Auto-Fix phase, and only after the user types `y`. A bare `/vibe-cartographer-vitals` invocation with every auto-fix declined is indistinguishable from the read-only-only behavior.

## Before You Start

- **Data contracts:** [`.agent/skills/vibe-cartographer-guide/references/data-contracts.md`](.agent/skills/vibe-cartographer-guide/references/data-contracts.md) — file locations, schemas, and atomic-write/append protocols. Vitals reads every file named in that doc. Do not write any of them from this command.
- **Friction triggers contract:** [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — the `/vibe-cartographer-vitals` section is deliberately empty. `/vibe-cartographer-vitals` does **not** call `vibe-cartographer-friction-logger.log()` in 1.5.0. Declines on future auto-fix prompts are the expected mode of interaction, not friction signal. Check #6 below audits the absence.
- **Schemas:**
  - [`.agent/skills/vibe-cartographer-guide/schemas/builder-profile.schema.json`](.agent/skills/vibe-cartographer-guide/schemas/builder-profile.schema.json) — check #3 validates the unified profile against this.
  - [`.agent/skills/vibe-cartographer-guide/schemas/friction.schema.json`](.agent/skills/vibe-cartographer-guide/schemas/friction.schema.json) — check #8 validates each `friction.jsonl` line against this.
  - [`.agent/skills/vibe-cartographer-guide/schemas/session-log.schema.json`](.agent/skills/vibe-cartographer-guide/schemas/session-log.schema.json) — referenced by checks #5 and #8 when reading session lines.
...
- **Session logger interface:** the `vibe-cartographer-session-logger` skill (`.agent/skills/vibe-cartographer-session-logger/SKILL.md`) — `start(command, project_dir)` returns the sessionUUID for this run; terminal `end(entry)` takes it back in at command completion. Vitals itself doesn't emit friction, but it still bookends with vibe-cartographer-session-logger sentinel + terminal entries like every other command.

## Session Logging

At workflow start, call `vibe-cartographer-session-logger.start("vitals", <project_dir>)` to get the sessionUUID. Hold it in memory for the duration of the command.

At workflow end (after the report prints **and** any auto-fix prompts have been resolved), call the vibe-cartographer-session-logger terminal-append procedure with:
...
Reference: [`.agent/skills/vibe-cartographer-guide/references/friction-triggers.md`](.agent/skills/vibe-cartographer-guide/references/friction-triggers.md) — section `/vibe-cartographer-vitals` is intentionally empty. Vitals does **not** call `vibe-cartographer-friction-logger.log()` in 1.5.0. User declines on auto-fix prompts are the **expected** mode of interaction, not friction. Logging them would flood `/vibe-cartographer-evolve` with noise about a user simply choosing not to apply a fix.

This rule holds for **all six** auto-fix actions below — declines (`n`) on any auto-fix prompt never produce a friction entry. Only auto-fix (b) writes to the friction log, and it does so on behalf of `vibe-cartographer-friction-logger.detect_orphans()` to emit `command_abandoned` entries for orphans the user has opted in to clean up. That is not a `/vibe-cartographer-vitals`-originated friction signal; it is retroactive emission of a friction type whose producer is documented as `vibe-cartographer-friction-logger.detect_orphans()` per `friction-triggers.md`.
...
Check #6 below explicitly audits that the `/vibe-cartographer-vitals` section of `friction-triggers.md` remains empty and that no `vibe-cartographer-friction-logger.log()` invocation exists in this workflow **as a SKILL-declared trigger**. The auto-fix (b) invocation lives inside the Auto-Fix Actions section, not in the "Friction Logging" section, and check #6 looks only at the latter — consistent with how it handles all other command workflows.

## Persona Adaptation

Vitals is a diagnostic, not a conversation. Persona still applies to the one interactive moment — the opening line before the report renders — but the report body itself is neutral (the boxed checks don't carry tone). Read `shared.preferences.persona` from `~/.gemini/profiles/builder.json`. Keep the opening to one sentence:
...
- **Architect:** "Structural sweep — eight checks across workflows, schemas, logs, and runtime context."
...
| What | Where |
|------|-------|
| Plugin root | the port's `.agent/` directory — the currently-installed port. Determine from this workflow file's own location (`.agent/workflows/`), walk up one level to `.agent/`. |
| Workflow files | `.agent/workflows/*.md` (13 expected) |
| Skill files | `.agent/skills/*/SKILL.md` (4 expected: guide, decay, session-logger, friction-logger) |
| Templates directory | `.agent/skills/vibe-cartographer-guide/templates/` |
| Schemas directory | `.agent/skills/vibe-cartographer-guide/schemas/` |
| Guide skill | `.agent/skills/vibe-cartographer-guide/SKILL.md` (for the anchored complements table used by check #4) |
| Friction triggers doc | `.agent/skills/vibe-cartographer-guide/references/friction-triggers.md` (check #6) |
| Scripts directory | `.agent/scripts/` (atomic-append-jsonl.js, atomic-write-json.js — check #1) |
| Architecture directory | `.agent/architecture/` (default-patterns, example-architecture, README) |
| Unified profile | `~/.gemini/profiles/builder.json` |
| Profiles directory | `~/.gemini/profiles/` (check #7) |
| Plugin data directory | `~/.gemini/antigravity/data/vibe-cartographer/` (check #7) |
| Sessions directory | `~/.gemini/antigravity/data/vibe-cartographer/sessions/` |
| Friction log | `~/.gemini/antigravity/data/vibe-cartographer-friction.jsonl` |
| Plugin manifest | `.agent/agent.json` (for version in banner) |
...
Read `.agent/agent.json`'s `"version"` field. Fall back to `"unknown"` on parse failure. Capture the current local ISO datetime for the banner.
...
Re-run /vibe-cartographer-vitals any time to re-check. For deeper structural proposals, see /vibe-cartographer-evolve.
```

Then do the vibe-cartographer-session-logger terminal append (see "Session Logging" above). No handoff — vitals is terminal, not a step in the sequential chain.
...
**(a) Read.** Enumerate every `.agent/workflows/*.md` (13) and every `.agent/skills/*/SKILL.md` (4). For each file, scan the body for markdown links and bare path references that point to other workflow/skill files — both relative paths (the `vibe-cartographer-session-logger` skill (`.agent/skills/vibe-cartographer-session-logger/SKILL.md`)) and root-absolute paths (`.agent/skills/vibe-cartographer-session-logger/SKILL.md`). Also include paths inside backtick code spans (e.g., `` `.agent/skills/vibe-cartographer-guide/SKILL.md` ``) and the `node .agent/scripts/<helper>.js` references to the carried atomic-write helpers in `.agent/scripts/`.
...
**(a) Read.** Same workflow-and-skill enumeration as check #1. Scan for references of the form `templates/<filename>` or `.agent/skills/vibe-cartographer-guide/templates/<filename>` or any path that resolves under `.agent/skills/vibe-cartographer-guide/templates/`. Also enumerate the templates directory so the check can report templates that exist-but-aren't-referenced (warn only; not a failure).
...
- ⚠ warn: a template exists on disk but no SKILL references it. List each as `<template_path>` under the "unreferenced templates" label. (This is a soft signal for `/vibe-cartographer-evolve` to consider pruning; not a structural failure.)
...
**Purpose:** the unified profile at `~/.gemini/profiles/builder.json` parses as JSON and validates against `builder-profile.schema.json`, with no unknown top-level or namespace-level fields.

**(a) Read.** Open `~/.gemini/profiles/builder.json`. Open `.agent/skills/vibe-cartographer-guide/schemas/builder-profile.schema.json`.

**(b) Evaluate.**
1. Profile missing entirely → ⚠ warn with message *"No unified profile yet — run `/vibe-cartographer-onboard` to create one. Subsequent vitals runs will validate it."* (Not a failure: first-time or un-onboarded users legitimately have no profile.)
...
**Purpose:** every anchored Pattern #13 complement in the `vibe-cartographer-guide` skill's anchored-complements table is present in the agent's current available skills/tools list.

**(a) Read.** Parse the "Anchored complements table" section of `.agent/skills/vibe-cartographer-guide/SKILL.md` — the markdown table whose first column is `Complement`. Extract the complement identifier from each row's first cell (strip backticks; normalize MCP wildcards like `mcp__plugin_playwright_playwright__*` to a prefix-match intent). Cross-reference against the agent's runtime available-skills list (the same list the agent uses to satisfy the Skill tool and the MCP tool surfaces).
...
- ⚠ warn: one or more anchored complements are not present in the available-skills list (and runtime context is complete). List each as `<complement_identifier> — not found in available skills`. Suggest: *"Either the builder removed that plugin, or the anchored table is stale. `/vibe-cartographer-evolve` can propose removing a defunct complement from the table; auto-fix (d) in a later vitals release will offer it inline."*
- **Fail-soft branch (runtime context incomplete):** box header still reads `Check #4 — Pattern #13 complement availability`, status is ⚠ warn, and the body is exactly: *"Could not verify N complements due to incomplete runtime context. Re-run /vibe-cartographer-vitals after restarting Claude Code."* where N is the count of anchored complements that could not be verified. Do not list individual complements in this branch — the signal is not reliable and listing them would create false evidence for `/vibe-cartographer-evolve` later.
...
**(a) Read.**
- Session log window: by default, the last 30 days of `~/.gemini/antigravity/data/vibe-cartographer/sessions/*.jsonl`. With `--full`, the complete history.
- Friction log: `~/.gemini/antigravity/data/vibe-cartographer-friction.jsonl`.

Parse each session-log line as JSON; silently skip malformed lines (check #8 owns repair, not this check). Count terminal entries only (outcome in `completed | abandoned | error | partial`) — sentinels without a terminal are counted by `vibe-cartographer-friction-logger.detect_orphans()`, not here.
...
3. **Otherwise** compute: `friction_per_session = friction_entries_in_window / terminal_entries_in_window`. Then evaluate against the thresholds:
   ...
   - `friction_per_session > 5.0` → **over-firing**: detection may be poisoning `/vibe-cartographer-evolve`. Warn.
...
The 5.0 upper bound and 0.05 lower bound are educated guesses per the spec's "Real-world friction-detection thresholds" open issue; `/vibe-cartographer-reflect` calibration will tune these in a later release.
...
**Purpose:** every `vibe-cartographer-friction-logger.log()` invocation in command workflows maps to a `friction-triggers.md` entry, and every `friction-triggers.md` entry has at least one corresponding invocation (or a documented exception).

**(a) Read.**
- `.agent/skills/vibe-cartographer-guide/references/friction-triggers.md` — parse out each per-command section and its table rows. For each row, extract the `friction_type` column value and the command name from the section heading (e.g., `## /vibe-cartographer-scope` → `scope`).
- Every chain/tool workflow at `.agent/workflows/*.md` — extract friction invocations declared by the workflow's **"Friction Logging"** section (the canonical list of triggers and friction types each workflow pledges to emit). This section is consistently structured across all chain workflows. For each bullet/row, extract the `friction_type: "<value>"` literal. Record `(workflow_name, friction_type_string)` where the command name is derived from the workflow filename (`vibe-cartographer-scope.md` → `scope`). **Do not** match `vibe-cartographer-friction-logger.log(` substrings outside the Friction Logging section — those appear in narrative prose ("this workflow does not call `vibe-cartographer-friction-logger.log()` …") and match spuriously.

**(b) Evaluate.** Build two sets:
- **Trigger map:** `{ (command, friction_type) | declared in friction-triggers.md }`.
- **Invocation map:** `{ (command, friction_type) | appears in the matching workflow's body }`. The command name is the workflow's filename stem (e.g., `.agent/workflows/vibe-cartographer-scope.md` → `scope`). Universal triggers from the "Universal triggers" section of `friction-triggers.md` are tagged to every command except the two documented exceptions — `/vibe-cartographer-vitals` and `/vibe-cartographer-friction` — whose per-command tables are explicitly empty by design.
...
- **Orphan triggers:** `(command, friction_type)` declared in the trigger map but with no matching invocation in the SKILL. Exclude two categories:
  - `command_abandoned` universally — emitted only by `vibe-cartographer-friction-logger.detect_orphans()`, not directly by command workflows.
  - Anything under the `/vibe-cartographer-vitals` or `/vibe-cartographer-friction` headings (both documented as empty tables).
...
**Vitals-specific assertion:** also verify that this file (`vitals/SKILL.md`)'s "Friction Logging" section declares **no** `friction_type` (it explicitly documents the absence). If a future edit adds one — despite the documented empty `/vibe-cartographer-vitals` table — surface it under orphan invocations as `vitals: declares <friction_type> in Friction Logging but /vibe-cartographer-vitals table is documented empty`. Same applies to `friction-log/SKILL.md` (the `/vibe-cartographer-friction` command) for the same reason.
...
- `~/.gemini/antigravity/profiles/` — list top-level entries.
- `~/.gemini/antigravity/data/vibe-cartographer/` — list top-level entries plus the contents of `sessions/`.
...
- ⚠ warn: one or more `.tmp` files found. List each as `<path> (modified <timestamp>)`. Suggest: *"These are leftover debris from an interrupted atomic write. A future `/vibe-cartographer-vitals` release will offer auto-fix (e) to delete them with confirmation. For now, inspect and `rm` manually if stale."*
...
**(a) Read.** Open `~/.gemini/antigravity/data/vibe-cartographer-friction.jsonl`. Open `.agent/skills/vibe-cartographer-guide/schemas/friction.schema.json`.
...
- ⚠ warn (special — no friction log): box body reads exactly: *"No friction log yet — feature available after first friction event."*
- ✗ fail: one or more lines failed. List each as `<line_no>: <failure_kind> — <detail>`. Suggest: *"A future `/vibe-cartographer-vitals` release will offer auto-fix (f) to rewrite the file dropping only malformed lines, with a `.bak` backup of the current file. For now, inspect the file manually — malformed lines near the end usually indicate a write that was interrupted mid-append."*
...
**Purpose.** Detect when Cart commands were executed via narrative orchestration (e.g., one chat session running multiple Cart commands as a chain, or an agent invoking Cart's SKILLs from outside Cart's own runtime context) without the vibe-cartographer-session-logger SKILL firing. Symptom: `process-notes.md` across recent projects records command invocations that have no matching entry in `~/.gemini/antigravity/data/vibe-cartographer/sessions/<date>.jsonl`. Without this check, `/vibe-cartographer-evolve` reads incomplete data and proposes against a partial picture.

**(a) Inputs.**
- Session log files at `~/.gemini/antigravity/data/vibe-cartographer/sessions/*.jsonl` (last 14 days by file mtime).
- `process-notes.md` files across the most recent 5 projects by `last_modified` (same set the `/vibe-cartographer-evolve` SKILL pulls per its "Read process-notes.md from recent projects" rule).
...
- ⚠ warn: list each project with missed runs as `<project_dir>: <N> Cart commands in process-notes without session-log entries (suggests orchestrator-level invocation outside Cart's runtime — vibe-cartographer-session-logger SKILL never fired).` Suggest: *"A future `/vibe-cartographer-vitals` release will offer auto-fix (g) to backfill synthetic session-log entries from process-notes summaries. For now, the data gap means `/vibe-cartographer-evolve` reads partial signal — known limitation."*
...
**(d) Fail-soft.** No process-notes files present → ✓ pass with body text *"No process-notes scanned — first-run state."* Session log dir empty → ⚠ warn (this is a vibe-cartographer-session-logger setup issue, not a coverage gap; reuse check #5's empty-log message pattern).
...
Where `<version>` is the `version` field from `.agent/agent.json` (or `"unknown"` if the manifest could not be read), and `<ISO-local-timestamp>` is the local-time ISO datetime captured at vitals start.
...
```
  ...
  └──────────────────────────────────────────────────────────────────┘
     ...
     Re-run /vibe-cartographer-vitals after restarting Claude Code.
```
...
**Apply.** Build the merged profile object in-memory, then write via `node .agent/scripts/atomic-write-json.js ~/.gemini/profiles/builder.json`. On success, post-apply line reads: `  ✓ applied: migrated plugins.app-project-readiness → plugins.vibe-cartographer (N keys moved, M merged).`

### Fix (b) — Orphan `command_abandoned` emission

**Trigger condition.** Check #5 (or an explicit `vibe-cartographer-friction-logger.detect_orphans()` side-pass invoked by vitals before the auto-fix phase — see note below) identified one or more `in_progress` sentinel entries in `sessions/*.jsonl` older than 24h with no matching terminal entry.

**Detection runs in this SKILL.** Vitals does not invoke `vibe-cartographer-friction-logger.detect_orphans()` inline — that's `/vibe-cartographer-onboard`'s role. But for the auto-fix phase, vitals re-derives the same orphan set locally from the session log scan it already performed for check #5, applying the >24h-old and no-matching-terminal rule. This keeps the check read-only: the orphan candidates are surfaced, not logged, until the user opts in.

**What it does.** For each orphan, emit a `command_abandoned` friction entry via `vibe-cartographer-friction-logger.log()` with fields: `original_command`, `original_timestamp` (the sentinel's timestamp), `sessionUUID`, `project_dir` (all pulled from the orphan sentinel). `confidence: "high"`, `symptom: "Sentinel recorded at <original_timestamp>, no terminal entry within 24h"`, `agent_guess_at_cause: null`, `complement_involved: null`.
...
**Apply.** Each emission goes through `vibe-cartographer-friction-logger.log(entry)`, which itself goes through `node .agent/scripts/atomic-append-jsonl.js`. Post-apply line counts the emissions actually written: `  ✓ applied: emitted <N> command_abandoned friction entries.`
...
**What it does.** For each confirmed-defunct complement, propose removing its row from the anchored-complements table in `.agent/skills/vibe-cartographer-guide/SKILL.md`. Per-complement `[y/n]` — never batch, since removing an anchored complement is a real semantic edit and the user should decide one at a time.

**Diff shown in prompt.** Render the current table row being removed plus the lines above and below for context.

```
  Check 4 reported: superpowers:old-brainstorm is anchored in the `vibe-cartographer-guide` skill
  but not present in the agent's available skills list.

  Proposed removal from skills/vibe-cartographer-guide/SKILL.md:

    | `superpowers:brainstorming`       | /vibe-cartographer-scope (deepening)   | Offer before round 3 |
  - | `superpowers:old-brainstorm`      | /vibe-cartographer-scope (legacy)      | Legacy — safe to drop |
    | `superpowers:verification-...`    | /vibe-cartographer-build (pre-commit)  | Offer post-fix       |

  Remove this row? [y/n]
```

**Apply.** Direct edit of `.agent/skills/vibe-cartographer-guide/SKILL.md` — the only non-profile file any auto-fix modifies. Read the file, remove exactly the matching table row (match by the complement identifier in column 1), write back. On success, post-apply line: `  ✓ applied: removed <complement_identifier> from anchored complements table.`
...
**Trigger condition.** Check #7 surfaced one or more `.tmp` files in `~/.gemini/antigravity/profiles/` or `~/.gemini/antigravity/data/vibe-cartographer/` (or its `sessions/` subdirectory).
...
```
  Found leftover .tmp file: ~/.gemini/profiles/builder.json.tmp
...
- The auto-fix phase itself never retries — one round per eligibility per `/vibe-cartographer-vitals` invocation. Re-run `/vibe-cartographer-vitals` to get another offer after the underlying condition is addressed.
...
- **Check 3** — ⚠ warn with "No unified profile yet — run `/vibe-cartographer-onboard` to create one" if the builder hasn't run `/vibe-cartographer-onboard`; ✓ pass post-onboard.
...
- **Check 6** — ✓ pass. The step-10 install wires friction-triggers.md and the command workflows consistently; the one intentional empty row (`/vibe-cartographer-vitals`) and one (`/vibe-cartographer-friction`) are documented and excluded from the orphan-trigger set.
...
A typical steady-state summary line after `/vibe-cartographer-onboard` and several sessions is `8 ✓  ·  0 ⚠  ·  0 ✗`.
...
**Staged:** `~/.gemini/profiles/builder.json` contains `plugins.app-project-readiness = { persona: "superdev" }` and no `plugins.vibe-cartographer` block.
...
6. `node .agent/scripts/atomic-write-json.js ~/.gemini/profiles/builder.json` with the new object on stdin. Exit 0.
...
4. Vitals calls `vibe-cartographer-friction-logger.log({ friction_type: "command_abandoned", original_command: "scope", original_timestamp: "2026-04-15T08:00:00-05:00", sessionUUID: "AAAA-...", project_dir: "app-readinessplugin", confidence: "high", symptom: "Sentinel recorded at 2026-04-15T08:00:00-05:00, no terminal entry within 24h", agent_guess_at_cause: null, complement_involved: null })`.
5. `vibe-cartographer-friction-logger.log` validates against `friction.schema.json`, then appends via `node .agent/scripts/atomic-append-jsonl.js ~/.gemini/antigravity/data/vibe-cartographer-friction.jsonl`. Exit 0.
...
**Staged:** `skills/vibe-cartographer-guide/SKILL.md` anchored table contains a row `| superpowers:fake-complement | /vibe-cartographer-scope | Offer before round 3 |`. Runtime available-skills list does **not** include `superpowers:fake-complement`. Runtime context is **complete** (Cart's own skills enumerable).
...
4. User types `y`.
5. Vitals reads `skills/vibe-cartographer-guide/SKILL.md`, removes exactly the matching row (identifier `superpowers:fake-complement` in column 1), writes back.
...
**Staged:** `~/.gemini/profiles/builder.json.tmp` exists, modified yesterday. No other `.tmp` files.

**Expected trace:**

1. Check #7 surfaces: `~/.gemini/profiles/builder.json.tmp (modified 2026-04-15T...)` (⚠ warn).
2. Auto-fix (e) eligibility: 1 file.
3. Prompt shows the full path and modification timestamp. User types `y`.
4. `fs.unlinkSync("~/.gemini/profiles/builder.json.tmp")` — succeeds.
5. Post-apply line: `  ✓ applied: deleted ~/.gemini/profiles/builder.json.tmp.`
