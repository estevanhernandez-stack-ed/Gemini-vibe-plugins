...
**Signals:**
- `.agent/agent.json` manifest (strongest signal — overrides any incidental web-framework signals)
...
- If a plugin persists data (e.g., a user profile at `~/.gemini/profiles/builder.json`), the Threat Model becomes Recommended: document what's stored, where, and how users can inspect/delete it.
