---
name: vibe-doc-session-logger
description: "Internal SKILL — not a user workflow. Two-phase append-only session log for Vibe Doc: a sentinel entry at command start (outcome=in_progress) and a terminal entry at command end, paired by sessionUUID. Invoked by every command at start and end. Part of Level 2 (session memory) of the Self-Evolving Plugin Framework."
---

# vibe-doc-session-logger — Sentinel + Terminal Session Log

Internal SKILL. Not a user-invocable slash command. Every Vibe Doc command calls `start()` at invocation and `end()` at completion. The two entries share a `sessionUUID` so `vibe-doc-friction-logger.detect_orphans()` can pair them.

## Before You Start

- **Schema:** [`../vibe-doc-guide/schemas/session-log.schema.json`](../vibe-doc-guide/schemas/session-log.schema.json) — JSON Schema Draft-07. The schema's `oneOf` splits sentinel (outcome=in_progress) from terminal (outcome in completed/abandoned/error/partial). Every write validates against this.
- **Atomic protocol:** all session log writes go through `node .agent/scripts/atomic-append-jsonl.js`. Never `>>` from a shell.
- **Orphan pairing:** the sentinel's `sessionUUID` is the load-bearing field. `vibe-doc-friction-logger.detect_orphans()` scans 7 days of session files looking for sentinels whose `(command, project_dir, sessionUUID)` has no matching terminal entry.

## Where the Log Lives

`~/.gemini/antigravity/data/vibe-doc/sessions/<YYYY-MM-DD>.jsonl`
...
- **plugin_version** — read from `.agent/agent.json`. If you can't determine it, use `"unknown"`.
- **command** — which command is running: `vibe-doc-scan`, `vibe-doc-generate`, `vibe-doc-check`, `evolve`.
...
- **friction_notes** — array of short strings. Human-facing recap. The actual structured friction signal goes to `friction.jsonl` via `vibe-doc-friction-logger.log()`.
...
Called by a workflow at invocation. Returns the `sessionUUID` the workflow must hold in memory until it calls `end()`.

**Arguments:**
- `command` — the command name (`vibe-doc-scan`, `vibe-doc-generate`, `vibe-doc-check`, `evolve`).
...
2. **Determine audit fields.**
   ...
   - `persona`: read from the unified profile at `~/.gemini/profiles/builder.json`. Pass through as-is; `null` if unset.
...
5. **Atomic append.** Pipe the JSON-stringified entry to:
   ```bash
   node .agent/scripts/atomic-append-jsonl.js ~/.gemini/antigravity/data/vibe-doc/sessions/<today>.jsonl
   ```
...
6. **Return the `sessionUUID`** to the caller. The workflow holds it in memory for the duration of the run and passes it back when calling `end()`.

## Procedure: `end(entry)`

Called by a workflow at completion, before the handoff to the next workflow. Takes the sessionUUID issued by `start()` plus the terminal fields that weren't known at start time.
...
**Failure handling:** session logging is instrumentation. A failed append logs a one-line warning to stderr and the command proceeds to handoff. The user never sees a vibe-doc-session-logger error.

## Namespace Isolation (Pattern #11)

This SKILL writes to exactly one place:

1. **`~/.gemini/antigravity/data/vibe-doc/sessions/<date>.jsonl`** — session log file (plugin-owned directory, append-only).

It does NOT write to `~/.gemini/profiles/builder.json`. That's the `update_unified_profile` step of `/vibe-doc-scan` and `/vibe-doc-generate` (which already exists in those workflows and writes only inside `plugins.vibe-doc.*`).
...
The session log is raw material for **Level 3** of the Self-Evolving Plugin Framework. `/vibe-doc-evolve` reads these entries (alongside `friction.jsonl`) to propose plugin improvements based on observed patterns.

The **sentinel pattern** lets `vibe-doc-friction-logger.detect_orphans()` distinguish "user abandoned the command" from "command never ran" — abandonment is friction signal worth surfacing; non-execution isn't.
