...
# /vibe-doc-generate — Generate Missing Documentation
...
Persona, posture, the unified-profile rules, and cross-plugin/Pattern #13 composition are always-on via `AGENTS.md`. Reference detail (state schema, CLI patterns, the documentation matrix, the breadcrumb-heuristics synthesis-question sets, the friction-trigger map) lives in the `vibe-doc-guide` skill at `.agent/skills/vibe-doc-guide/SKILL.md` — load it when you need a per-doc-type question set or to validate a state write. Then follow this workflow.

## Session Logging

At workflow start — before reading state.json or any project files — call `vibe-doc-session-logger.start("vibe-doc-generate", <project_dir>)` (load the `vibe-doc-session-logger` skill at `.agent/skills/vibe-doc-session-logger/SKILL.md`) to get the sessionUUID. Hold it in memory for the duration of this workflow. Pass it to every `vibe-doc-friction-logger.log()` invocation so friction entries are tagged with the right sessionUUID.

At workflow end — after the generated doc has been written to disk and the user has either accepted the result or moved on — call `vibe-doc-session-logger.end(entry)` with the **same sessionUUID** returned by `start()`. Set `outcome: "completed"` if the doc generated and was accepted, `"partial"` if it generated but the user exited before review, `"abandoned"` only if the workflow exited before any generation. Populate `friction_notes`, `key_decisions` (e.g., "picked Required tier first", "skipped intake questions for sections agent had high confidence on"), `artifact_generated: <path-to-the-doc>`, and `complements_invoked` from what actually happened (especially `context7:*` calls).

## Friction Logging

Reference: `.agent/skills/vibe-doc-guide/references/friction-triggers.md` — section `/vibe-doc-generate`. Invoke `vibe-doc-friction-logger.log()` at exactly these triggers, with exactly these confidence levels:
...
Every `log()` call passes the sessionUUID returned by `vibe-doc-session-logger.start()` at the top of this workflow so entries cluster under this run.
...
1. **Read the project files directly** — README, AGENTS.md, package.json, SKILL files, source entry points, git history, CI configs
...
The CLI (`vibe-doc generate <doctype>`) still produces the deterministic scaffold. This workflow layers intelligence on top: same scaffold, but the agent keeps going and fills it in from the codebase before handing off to the user.
...
if [ ! -f "<project-path>/.vibe-doc/state.json" ]; then
  echo "No project profile found. Run /vibe-doc-scan first."
  exit 1
fi
```

If state doesn't exist, redirect to the **/vibe-doc-scan workflow** and exit.
...
|----------|------------------|
| **readme** | `package.json`, `AGENTS.md`, any existing `README.md`, main source entry file (e.g., `src/index.ts`), `docs/` summaries |
...
| **skill-command-reference** | every `skills/*/SKILL.md`, every `commands/*.md`, `.agent/agent.json` |
...
| **adr** | `AGENTS.md`, commit messages with "decision:" or "arch:" prefixes, any `docs/adr/` or `docs/decisions/` folder |
...
After any successful generation (single or parallel), update the `plugins.vibe-doc` namespace in `~/.gemini/profiles/builder.json` — but **only if the file already exists**.

1. Read `~/.gemini/profiles/builder.json`. If it doesn't exist or isn't valid JSON, skip this step.
...
For these, default to a **short autonomous pass** (fill only what's obviously there) and spend most of the time in the interview phase. Lean on the synthesis questions from `.agent/skills/vibe-doc-guide/references/breadcrumb-heuristics.md`.
...
- Prefer summary files (READMEs, AGENTS.md, SKILL.md) over large source files
...
- `.vibe-doc/state.json`'s `classification` or `gapReport` blocks — those are owned by the scan/check workflows

---

## Synthesis Questions Reference

When the interview phase is needed, question sets per doc type live in `.agent/skills/vibe-doc-guide/references/breadcrumb-heuristics.md`. Each breadcrumb's `gapQuestions` field is a pre-written list of targeted questions for that doc type — use them as a starting point and adapt to what you already filled in.

---

**Last updated:** 2026-04-15 | **Version:** 2.0 (autonomous-first; Antigravity port)
