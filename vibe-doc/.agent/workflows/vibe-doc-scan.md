...
# /vibe-doc-scan — Scan for Documentation Gaps

Conversational pipeline to scan a project, classify its type, and produce a documentation gap report.

Persona, posture, the unified-profile rules, and cross-plugin/Pattern #13 composition are always-on via `AGENTS.md`. Reference detail (state schema, CLI patterns, the classification taxonomy / documentation matrix / breadcrumb heuristics, the friction-trigger map) lives in the `vibe-doc-guide` skill at `.agent/skills/vibe-doc-guide/SKILL.md` — load it when you need to validate a state write or pull a classification rule. Then follow this workflow.

## Session Logging

At workflow start — before the entry gate prompt — call `vibe-doc-session-logger.start("vibe-doc-scan", <project_dir>)` (load the `vibe-doc-session-logger` skill at `.agent/skills/vibe-doc-session-logger/SKILL.md`) to get the sessionUUID. Hold it in memory for the duration of this workflow. Pass it to every `vibe-doc-friction-logger.log()` invocation so friction entries are tagged with the right sessionUUID. Then call `vibe-doc-friction-logger.detect_orphans()` once to convert any backlog of abandoned commands into friction entries.

At workflow end — after the gap report has been presented and the user has either chosen a next step or the unified profile write completed — call `vibe-doc-session-logger.end(entry)` with the **same sessionUUID** returned by `start()`. Set `outcome: "completed"` if the full flow ran, `"partial"` if the user exited mid-flow but the scan completed, `"abandoned"` only if the workflow exited before the scan ran. Populate `friction_notes`, `key_decisions` (e.g., "chose Path A intake", "confirmed ClaudeCodePlugin classification"), `artifact_generated: ".vibe-doc/state.json"`, and `complements_invoked` from what actually happened.

## Friction Logging

Reference: `.agent/skills/vibe-doc-guide/references/friction-triggers.md` — section `/vibe-doc-scan`. Invoke `vibe-doc-friction-logger.log()` at exactly these triggers, with exactly these confidence levels:
...
- **User runs `/vibe-doc-generate` or `/vibe-doc-check` from the same project before this scan's terminal entry lands** → caught at next-command time as `sequence_revised`, `confidence: "medium"`. This workflow itself doesn't log it; the next workflow detects an orphan-followed-by-different-command pattern.
...
If the user picks `[generate]`, hand off to the `/vibe-doc-generate` workflow. If they pick `[check]`, hand off to `/vibe-doc-check`.
...
After a successful scan, update the `plugins.vibe-doc` namespace in `~/.gemini/profiles/builder.json` — but **only if the file already exists** (another plugin like Vibe Cartographer creates it during onboarding).

1. Read `~/.gemini/profiles/builder.json`. If it doesn't exist or isn't valid JSON, skip this step entirely.
...
**Last updated:** 2026-04-11 | **Version:** 1.0 (Antigravity port)
