...
# /vibe-doc-check — Deployment-Readiness Documentation Check

Simple CI-safe validation: verify that Required documentation exists and is current.

Run `npx vibe-doc check` in the user's project directory. Present pass/fail results clearly. If docs are missing or stale, suggest running `/vibe-doc-generate` to fix the gaps.

Persona, posture, and the unified-profile rules are always-on via `AGENTS.md`. Reference detail (state schema, CLI patterns, output formatting, the documentation matrix) lives in the `vibe-doc-guide` skill at `.agent/skills/vibe-doc-guide/SKILL.md` — load it if you need to validate which Required docs apply to a classification. Then follow this workflow. This workflow writes no files.
...
```
[regenerate] → Run /vibe-doc-generate to fix gaps
[scan] → Update project profile with /vibe-doc-scan
[review] → I'll show you what's in docs/generated/
[exit] → Done with check (I'll report failure to CI)
```

**If user picks "regenerate":**
- Hand off to the `/vibe-doc-generate` workflow with pre-selected stale/missing docs

**If user picks "scan":**
- Hand off to the `/vibe-doc-scan` workflow
...
The `vibe-doc check` command returns standard exit codes:
...
- name: Report Status
  ...
  run: |
    ...
    echo "Regenerate docs before merging: run /vibe-doc-generate"
    exit 1
```
...
This usually means:
  • No .vibe-doc/state.json file (run /vibe-doc-scan first)
  • Corrupted state file
  • File system permissions issue

Next steps:
  1. Run /vibe-doc-scan to create/repair project profile
  2. Then re-run check
```
...
**No files created or modified by this workflow.**
