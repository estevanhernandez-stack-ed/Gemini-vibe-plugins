...
> **Prerequisite — the engine is a separate install.** This workflow orchestrates
> an external Python CLI; it does **not** bundle it. The `vibe-insights` engine is a
> standalone, pip-installable tool (Python 3.11+), the same binary the Claude Code
> skill ran. Install it once where the workspace can reach it:
> `python -m pip install "git+https://github.com/estevanhernandez-stack-ed/vibe-insights"`,
> which puts `vibe-insights` on PATH (and `python -m vibe_insights.cli` works
> regardless). If the binary isn't found, surface the install line and stop — don't
> try to reimplement the analytics inline. The engine is deterministic and offline
> (no LLM, no MCP); this workflow is where the LLM lives.

## Steps

1. **Ensure the engine, then config.** First confirm the external `vibe-insights`
   binary is installed (see the prerequisite above) — if not, surface the `pip install`
   line and stop. Then, if `~/.vibe-insights/config.json` is missing, run
   ...
   (`{"source": "none"|"file"|"mcp"}`). (`~/.vibe-insights/` is the engine's own
   data dir — platform-agnostic, not a Claude/Antigravity path; leave it as-is.)
