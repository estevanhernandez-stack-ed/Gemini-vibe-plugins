...
Takes a brief (a markdown file path OR an inline string) and runs Ptolemy's build phase directly — no candidate clustering, no scoring, no Atlas-history check. The user already knows what they want; `/vibe-iterate-ship` is the express lane.
...
- **Atlas write is mandatory.** Every `/vibe-iterate-ship` invocation that opens a PR (or that the user explicitly declines mid-build) writes an Atlas entry with `mode: "ship"`.
...
At workflow start — load the `vibe-iterate-session-logger` skill (`.agent/skills/vibe-iterate-session-logger/SKILL.md`) and run `start("ship", <project_dir>)` to get a sessionUUID. Hold it in memory; pass it to every `vibe-iterate-friction-logger.log()` invocation.
...
Honor the friction trigger map at `.agent/skills/vibe-iterate-guide/references/friction-triggers.md` — section `/vibe-iterate-ship` — for which friction types to log at which confidence. Note: a user request to skip the test run is a HIGH-confidence `default_overridden` signal.
...
- **Don't ingest signals.** That's the banner modes' job. `/vibe-iterate-ship` is the express lane — if the user wanted ingestion, they'd have invoked `feature-add`.
...
- Sibling: `/vibe-iterate-upgrade` — express lane for library bumps specifically
