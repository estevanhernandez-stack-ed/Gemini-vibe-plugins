...
Read-only. Does NOT bump anything — that's `/vibe-iterate-upgrade`'s job. The output is decision-support: "is this bump safe? does it unblock anything? is there a known migration path?"

## Hard rules

- **Read-only.** No package installs, no version bumps, no commits. The only side effect is a write to the radar cache (under `framework_releases[]`) so subsequent `/vibe-iterate-radar` calls have fresh data — this is opt-in via the `--cache` flag, default off for the standalone case.
...
- **Project state** — `package.json` (or stack equivalent: `pyproject.toml`, `Cargo.toml`, `go.mod`). If no manifest file is present, surface: *"No package manifest found. /vibe-iterate-scan-releases needs a package.json (or pyproject.toml / Cargo.toml / go.mod) to know what's pinned."*
...
Read existing cache (if any), update only `framework_releases[]` and `refreshed_at`, write back. Validate against `.agent/skills/vibe-iterate-guide/schemas/radar-cache.schema.json` before writing.

## When invoked internally

By `feature-add` mode: *"is there a fresh framework feature that unblocks the highest-impact item?"* — `feature-add` calls `/vibe-iterate-scan-releases --silent` and reads the structured result.

By `/vibe-iterate-upgrade` mode: pre-flight check before bumping a single package. `/vibe-iterate-upgrade` calls `/vibe-iterate-scan-releases <package> --silent` to get the version diff and codemod availability before doing the bump.
...
- Sibling sidecar: `/vibe-iterate-upgrade` — does the actual bump
- Sibling sidecar: `/vibe-iterate-radar` — broader scan including competitors and Product Hunt
