---
description: "Run when the user says /spy <url> and wants a one-shot competitive read on a single URL. Fetches and reads the URL (changelog, what's-new, landing page), outputs a structured read of what they shipped, what they emphasize, gaps, and overlap with the user's product."
---
...
- /competitive — ranks these gaps by strategic relevance and proposes one to ship
- /rate "<idea inspired by this>" — sanity-check before committing
- /radar — refresh the broader radar to put this in context
```
...
- Sibling sidecar: `/radar` — broader scheduled scan across all competitors
- Banner mode: `/competitive` — invokes `/spy` internally per URL
