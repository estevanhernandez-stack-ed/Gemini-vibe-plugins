...
Scans every URL in `config.competitors[]` via `/vibe-iterate-spy --silent`, diffs the union of competitor features against what the user's product ships today, identifies gaps, **ranks gaps by strategic relevance — not parity**, picks one, and ships it.
...
At workflow start — load the `vibe-iterate-session-logger` skill (`.agent/skills/vibe-iterate-session-logger/SKILL.md`) and run `start("competitive", <project_dir>)` to get a sessionUUID. Hold it in memory; pass it to every `vibe-iterate-friction-logger.log()` invocation.
...
Honor the friction trigger map at `.agent/skills/vibe-iterate-guide/references/friction-triggers.md` — section `/vibe-iterate-competitive` — for which friction types to log at which confidence.
...
For each URL in `config.competitors[]` (plus `--url` if passed), call `/vibe-iterate-spy <url> --silent`. Capture each spy output as a structured object with `shipped_features[]`, `positioning`, `audience`, `gaps_left_open`, etc.

If `/vibe-iterate-spy` fails for a URL (404, paywall, network), surface it once at the bottom of the diff — don't fail the whole mode.
...
For each gap, also call `/vibe-iterate-rate "<gap-as-iteration>" --silent` to get effort + regression-risk + user-trust-impact scores. Strategic-relevance label is the headline; the `/vibe-iterate-rate` scores inform feasibility.

### Step 7 — pick the top match-or-differentiate gap

Among `match` and `differentiate` gaps (skip `decline` for picking purposes), sort by `/vibe-iterate-rate` total score descending. Top entry is the recommendation.
...
Same as feature-add step 9 — invoke `/vibe-iterate-ship`-style build, OR delegate to Cart's `/scope → /prd → /spec → return → build`.
...
```json
{
  ...
  "rationale": "Strategic relevance: <match | differentiate>. Competitors with this feature: <urls>. <if differentiate: our version: ...>. Cart-detection: <light | delegated | declined>. Total /vibe-iterate-rate score: <N>/25.",
...
- Sidecars called: `/vibe-iterate-spy`, `/vibe-iterate-rate`, `/vibe-iterate-ship` (for build phase)
- Cart-detection: AGENTS.md § Cart-detection
- Atlas conventions: `.agent/skills/vibe-iterate-guide/references/atlas-conventions.md`
- Sibling: `/vibe-iterate-feature-add` — multi-source candidate-driven; this mode is competitor-driven only
