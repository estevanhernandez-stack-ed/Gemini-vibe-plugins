---
description: "Run when the user says /upgrade <package> and wants to surgically bump one library to its latest version with codemods if available. Reads release notes via /scan-releases, runs the codemod if one exists, runs the test suite, ships the bump as one PR with an Atlas entry."
---

# /upgrade <package> — surgical library bump
...
At workflow start — load the `session-logger` skill (`.agent/skills/session-logger/SKILL.md`) and run `start("upgrade", <project_dir>)` to get a sessionUUID. Hold it in memory; pass it to every `friction-logger.log()` invocation.
...
Honor the friction trigger map at `.agent/skills/guide/references/friction-triggers.md` — section `/upgrade` — for which friction types to log at which confidence. Note: a user opting for a major bump, requesting test-skip, or rolling back are all HIGH-confidence signals.
...
If `$1` is absent, prompt: *"Which package? Pass the package name (e.g., next, react, tailwindcss). For options, see /scan-releases first."*
...
- Sibling: `/scan-releases` — pre-flight version diff (called internally)
- Sibling: `/ship` — express lane for non-upgrade iterations
