...
- **Read-only on the source data.** `/vibe-iterate-evolve` does not edit `~/.gemini/antigravity/data/vibe-iterate/` files. Sessions and friction logs are append-only; `/vibe-iterate-evolve` reads them.
...
- **Output target:** `docs/proposed-changes.md` in the vibe-iterate port repo (this file). If the file already exists from a prior `/vibe-iterate-evolve` run, append a new section with today's date — don't overwrite.

If neither source data file exists (fresh install with no usage), surface: *"No session or friction data yet. `/vibe-iterate-evolve` learns from your past `vibe-iterate` runs — invoke a banner mode a few times, then re-run `/vibe-iterate-evolve`."* and exit.
...
### Step 2 — call `vibe-iterate-friction-logger.detect_orphans()`
...
Per the `vibe-iterate-friction-logger` skill (`.agent/skills/vibe-iterate-friction-logger/SKILL.md`).
...
**(b) Trigger map edit**
- File: `.agent/skills/vibe-iterate-guide/references/friction-triggers.md`
- Trigger to add/remove/re-tune: name the row
- Suggested change: new confidence level OR new trigger condition
- Evidence: as above

**(c) Rubric edit**
- File: where the rubric lives (e.g., `/vibe-iterate-rate`'s scoring rubric, `/vibe-iterate-ux-polish`'s user-trust-impact tiers)
...
> Each section is a `/vibe-iterate-evolve` run output. Reviewer (Este) applies, defers, or declines per proposal.

---

## /vibe-iterate-evolve run — YYYY-MM-DD
...
## /vibe-iterate-evolve run — <next date>
...
```
...
- **Don't propose changes outside the port.** `/vibe-iterate-evolve` proposes edits to `.agent/` and `docs/` within the vibe-iterate port. It does NOT propose edits to host projects' code or other plugins.
- **Don't read project-level files.** `/vibe-iterate-evolve` is plugin-meta. It reads `~/.gemini/antigravity/data/vibe-iterate/`, NOT `.vibe-iterate/atlas.jsonl` (that's project state).

## Cross-references

- Session log SKILL: the `vibe-iterate-session-logger` skill (`.agent/skills/vibe-iterate-session-logger/SKILL.md`)
- Friction log SKILL: the `vibe-iterate-friction-logger` skill (`.agent/skills/vibe-iterate-friction-logger/SKILL.md`)
- Trigger map: `.agent/skills/vibe-iterate-guide/references/friction-triggers.md`
