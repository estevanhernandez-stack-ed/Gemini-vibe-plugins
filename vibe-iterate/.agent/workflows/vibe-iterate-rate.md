...
See the **user-trust-impact rubric** in `the `/vibe-iterate-ux-polish` workflow` for the full version of this rubric — `/vibe-iterate-rate` uses the same anchors.
...
When a banner mode (`feature-add`, `competitive`, `ux-polish`, `bug-bash`) calls `/vibe-iterate-rate` to rank candidates, the calling mode is responsible for:
- Passing each candidate through `/vibe-iterate-rate`
...
- Rubric reference: `/vibe-iterate-ux-polish` — full user-trust-impact tiers
