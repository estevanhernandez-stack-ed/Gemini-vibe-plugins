...
At workflow start — load the `vibe-iterate-session-logger` skill (`.agent/skills/vibe-iterate-session-logger/SKILL.md`) and run `start("ux-polish", <project_dir>)` to get a sessionUUID. Hold it in memory; pass it to every `vibe-iterate-friction-logger.log()` invocation.
...
Honor the friction trigger map at `.agent/skills/vibe-iterate-guide/references/friction-triggers.md` — section `/vibe-iterate-ux-polish` — for which friction types to log at which confidence.
...
Use the **user-trust-impact rubric** (also used by `/vibe-iterate-rate`):
...
For each rough patch, also estimate effort (1-5, inverse — same as `/vibe-iterate-rate`): how big is the diff to fix this one rough patch?
...
Same as `/vibe-iterate-ship` build phase: pre-flight tests → implement the fix → post-flight tests → open PR.
...
🤖 Generated with [vibe-iterate](https://github.com/estevanhernandez-stack-ed/vibe-iterate) /vibe-iterate-ux-polish
```
...
- Rubric source-of-truth: this SKILL (cited by `/vibe-iterate-rate`)
- Sidecar called: `/vibe-iterate-rate` (for effort scoring per patch), `/vibe-iterate-ship` (for build phase)
