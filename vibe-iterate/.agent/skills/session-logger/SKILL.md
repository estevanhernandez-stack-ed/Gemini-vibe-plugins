...
Internal skill. Not a user-invocable workflow (no `.agent/workflows/` entry). Every vibe-iterate banner-mode workflow (`/feature-add`, `/competitive`, `/ux-polish`, `/bug-bash`) and `/bootstrap` loads this skill and runs `start()` at invocation and `end()` at completion. The two entries share a `sessionUUID` so `friction-logger.detect_orphans()` can pair them.
...
- **No write permission on `~/.gemini/antigravity/data/`** → log fails silently. The workflow continues. The user never sees a session-logger error.
...
- User-facing SKILL: [`../../workflows/evolve-iterate.md`](../../workflows/evolve-iterate.md) — reads sessions + friction, proposes improvements
