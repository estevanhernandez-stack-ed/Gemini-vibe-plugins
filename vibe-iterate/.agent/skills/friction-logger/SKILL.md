...
description: "Internal skill — not a user workflow. Append-only friction capture for vibe-iterate. Loaded by every banner-mode workflow + bootstrap at the trigger points listed in .agent/skills/guide/references/friction-triggers.md. Implements Pattern #6 (Friction Log) from the Self-Evolving Plugin Framework. Antigravity port — logs to ~/.gemini/antigravity/data/vibe-iterate/."
---
...
- User-facing SKILL: [`../../workflows/evolve-iterate.md`](../../workflows/evolve-iterate.md) — reads friction + sessions, proposes improvements
