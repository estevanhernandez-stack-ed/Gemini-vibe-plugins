...
- [`references/friction-triggers.md`](references/friction-triggers.md) — when each workflow logs which friction type at which confidence (per Pattern #6, consumed by the vibe-iterate-friction-logger skill).
...
Every banner mode (`/vibe-iterate-feature-add`, `/vibe-iterate-competitive`, `/vibe-iterate-ux-polish`, `/vibe-iterate-bug-bash`) and `/vibe-iterate-bootstrap` invokes:
- [`../vibe-iterate-session-logger/SKILL.md`](../vibe-iterate-session-logger/SKILL.md) — sentinel + terminal session entries, paired by sessionUUID, written to `~/.gemini/antigravity/data/vibe-iterate/sessions/<date>.jsonl`.
- [`../vibe-iterate-friction-logger/SKILL.md`](../vibe-iterate-friction-logger/SKILL.md) — append-only friction entries written to `~/.gemini/antigravity/data/vibe-iterate/friction.jsonl`.
...
|---|---|---|
| Antigravity scheduled-task / cron | Optional | Powers the weekly radar refresh; `/vibe-iterate-radar` supports manual refresh meanwhile |
