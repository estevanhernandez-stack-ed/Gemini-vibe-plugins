...
- **One entry per banner-mode or sidecar invocation that ships, rejects, or queues something.** Read-only sidecars (`/vibe-iterate-radar`, `/vibe-iterate-spy`, `/vibe-iterate-scan-releases`, `/vibe-iterate-rate`) do NOT write Atlas entries.
