...
> Antigravity port note: section headings name the **workflow** that logs the friction (slash form, e.g. `/vibe-iterate-feature-add`). In the Claude Code original these were skills invoked as `/vibe-iterate:<cmd>`. The mapping is 1:1 — only the invocation syntax changed.

Source of truth for `vibe-iterate-friction-logger.log()` calls across vibe-iterate. One section per command. Each row names the trigger condition + the `friction_type` enum value + the fixed `confidence` level.

**Rule:** confidence levels here are NOT agent-tunable. If a trigger feels mis-tuned, fix it here (and let `/vibe-iterate-evolve` propose the change) — don't override at log time.
...
## `/vibe-iterate-bootstrap` workflow
...
## `/vibe-iterate-feature-add` workflow
...
## `/vibe-iterate-competitive` workflow
...
| `/vibe-iterate-spy` fails for >50% of competitor URLs | `complement_rejected` | `low` | Soft signal — competitor pages are paywalled / blocked; `complement_involved: "WebFetch"` |
| User pauses at gap-selection step | `default_overridden` | `medium` | |

---

## `/vibe-iterate-ux-polish` workflow
...
## `/vibe-iterate-bug-bash` workflow
...
| User asks agent to skip the regression test | `default_overridden` | `high` | Hard rule violation requested; CRITICAL signal for `/vibe-iterate-evolve` |
| User declines Cart-delegation upsell on a heavy bug fix | `complement_rejected` | `high` | `complement_involved: "vibe-cartographer"` |

---

## `/vibe-iterate-ship` workflow
...
## Sidecar workflows (`/vibe-iterate-radar`, `/vibe-iterate-spy`, `/vibe-iterate-scan-releases`, `/vibe-iterate-rate`)

These are read-only and short-lived. **Do NOT call vibe-iterate-session-logger or vibe-iterate-friction-logger** from them. Sidecars are ephemeral spot-tools; the friction signal isn't worth the entry cost.
...
Each command SKILL has a "Session + friction logging" section that says: "Honor the trigger map at [`../vibe-iterate-guide/references/friction-triggers.md`](friction-triggers.md). Call `vibe-iterate-friction-logger.log()` at exactly these triggers, with exactly the listed confidence."
