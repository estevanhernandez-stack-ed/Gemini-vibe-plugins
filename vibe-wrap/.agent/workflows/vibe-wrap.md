---
description: "Run when the user says `/vibe-wrap` and wants a session-end handoff doc that reads the breadcrumb trail sibling vibe plugins already left, surfaces what shipped + what's uncommitted + what's unpushed, and gates commit + push interactively. Multi-repo aware (read wide, mutate narrow): 'What shipped' + 'Still unpushed' span every sibling repo that had commits in the session window, while the commit/push gates stay scoped to the current repo. Reads breadcrumbs, sibling session-logs / friction / wins, git state across repos, and the active decision-log backend. Writes a markdown wrap doc to `docs/session-wraps/<ts>.md` (fallback `.vibe-wrap/wraps/<ts>.md`) and prints inline. Bumper-lanes invariant — every gate defaults to no-action and has a clear skip path. Flags: `--inline-only`, `--bridge`, `--session-window <hours>`, `--repos <p1,p2,...>`, `--repo-roots <dir>`, `--no-multi-repo`."
---

# /vibe-wrap — Read the trail, render the handoff, gate the rest

The plugin's main command — the user-facing session wrap. Read the breadcrumb trail the toolkit already left, surface what shipped + what's uncommitted + what's unpushed, render a durable handoff doc, then surface the commit / push / decision-log / bridge gates interactively. **The doc always lands. The gates are bumpers, never walls.**

Persona, posture (the bumper-lanes invariant), voice, namespace isolation, and ecosystem-aware composition are always-on via `AGENTS.md`. Situational reference detail (the per-gate contract, secret patterns, the decision-log backend contract, the breadcrumb contract, the friction-trigger map) lives in the `vibe-wrap-guide` skill (`.agent/skills/vibe-wrap-guide/SKILL.md`) and the script-side `.agent/scripts/references/` — load them when a gate needs validating. Then follow this workflow.

## Before you start

- **Session logging.** Call `vibe-wrap-session-logger.start("wrap", <cwd>)` at workflow start, `vibe-wrap-session-logger.end(<entry>)` at workflow end. Per the `vibe-wrap-session-logger` skill (`.agent/skills/vibe-wrap-session-logger/SKILL.md`).
- **Persona + mode.** Read `shared.preferences.persona` and `shared.preferences.mode` from the unified profile per `AGENTS.md § Persona` + the guide skill's `references/persona-adaptation.md`. Persona shapes the prose voice; mode shapes pacing. The bumper-lanes invariant never changes.
- **Hard dependency: Python 3.11+.** If the runtime is older or absent, fail at workflow start with a clear next step. Git CLI is a soft dependency — if absent, the wrap still renders from breadcrumbs + sibling reads + decision-log, with a one-line note that git state is unavailable.
- **The division of labor.** `.agent/scripts/render-wrap.py` does the deterministic read + render + gate-state computation. It performs **no mutations**. This workflow drives the interactive gestures (commit, push, decision-log append, bridge) based on the gate state the script reports. Keep that boundary — the script is reproducible; the gates are conversational.
...
Before rendering, resolve the decision-log backend. The render script is read-only and will report `backend: "pending"` if no config exists — it never triggers the interactive picker. **This workflow owns the first-run gesture.**

At workflow start, import the dispatcher and call `active_backend(cwd)`. If it returns `None` (first-run pending) AND the session is interactive:

- Run the first-run picker per [`.agent/scripts/references/decision-log-backends.md`](../scripts/references/decision-log-backends.md) § First-run UX. Four options, recommended default labeled, `[skip — disabled]` as the fourth choice.
...
```
python .agent/scripts/render-wrap.py \
...
4. Assembles the markdown wrap from [`.agent/scripts/assets/wrap-template.md`](../scripts/assets/wrap-template.md).
...
**Print the rendered wrap doc inline to the user.** Then parse the gate-state JSON block — do not show that block to the user; it's the workflow's control channel.
...
See [`.agent/scripts/references/gate-design.md`](../scripts/references/gate-design.md) § Read wide, mutate narrow for the boundary contract.

## Step 3 — Gates (interactive, default no-action)

Read [`.agent/scripts/references/gate-design.md`](../scripts/references/gate-design.md) for the full per-gate contract. Surface each gate only when the gate-state block marks it `eligible: true`. **Every gate defaults to no-action. Pressing enter or typing `n` skips it.**
...
5. **Secret-pattern path:** for each entry in `gates.commit.secret_matches`, fire an additional `commit despite secret-pattern match? [y/N]` per [`.agent/scripts/references/secret-patterns.md`](../scripts/references/secret-patterns.md). Default `N` excludes the matched file. Record any override in the wrap doc's notes.
...
- Call `vibe-wrap-session-logger.end()` with `complements_invoked` listing each sibling whose state the wrap pulled from, plus the gates the user accepted.
...
Target <10s for the non-interactive portion: trail reader ≤4s, decision-log read ≤2s, render ≤1s. Gates are interactive and not budgeted. If a subsystem ships over budget against a real session, flag it for `/vibe-wrap-evolve`.

## Friction triggers

Per [`../skills/vibe-wrap-guide/references/friction-triggers.md`](../skills/vibe-wrap-guide/references/friction-triggers.md) § /vibe-wrap. Fire `vibe-wrap-friction-logger.log()` at the documented trigger points — gate declined after the doc surfaced a high-signal commit, repeated re-runs in one session, first-run picker abandoned, etc. Confidence is fixed per trigger; never override at log time.

## Reference

- [`.agent/scripts/render-wrap.py`](../scripts/render-wrap.py) — the render core (read + render + gate state).
- [`.agent/scripts/assets/wrap-template.md`](../scripts/assets/wrap-template.md) — the markdown template.
- [`.agent/scripts/read-breadcrumbs.py`](../scripts/read-breadcrumbs.py) · [`.agent/scripts/read-sibling-state.py`](../scripts/read-sibling-state.py) · [`.agent/scripts/git-state.py`](../scripts/git-state.py) — trail readers.
- [`.agent/scripts/multi-repo-state.py`](../scripts/multi-repo-state.py) — multi-repo git-state aggregator (read wide). Discovers sibling repos + reports per-repo state; calls `git-state.py`'s single-repo core per repo.
- [`.agent/scripts/decision-log/__init__.py`](../scripts/decision-log/__init__.py) — decision-log dispatcher.
- [`.agent/scripts/references/gate-design.md`](../scripts/references/gate-design.md) — bumper-lanes invariant per gate.
- [`.agent/scripts/references/secret-patterns.md`](../scripts/references/secret-patterns.md) — the patterns that trigger the secrets warning.
- [`.agent/scripts/references/decision-log-backends.md`](../scripts/references/decision-log-backends.md) — backend contract, config precedence, first-run UX.
- [`.agent/scripts/references/breadcrumb-contract.md`](../scripts/references/breadcrumb-contract.md) — schema + plant contract for sibling authors.
