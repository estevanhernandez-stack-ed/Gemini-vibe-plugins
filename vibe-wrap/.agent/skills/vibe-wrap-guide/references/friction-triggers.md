...
Source of truth for "when does each vibe-wrap command log which friction type." Every command SKILL references this doc. The `vibe-wrap-friction-logger` SKILL writes the actual entries via the calling SKILL — this file is the contract those calls honor.

## How to read this file

Each section covers one command. Within a section, a markdown table lists every condition under which that command should call `vibe-wrap-friction-logger.log()`, the friction type it emits, the default confidence, and any required-field notes.
...
`/vibe-wrap-evolve` weighting at high/medium/low: `1.0 / 0.6 / 0.3`. Same weighting Cart uses.

## Defensive defaults (apply to every trigger)

These four discipline rules apply across every `vibe-wrap-friction-logger.log()` call vibe-wrap makes. Honor them by default; never override at log time.
...
2. **Quoted-prior discipline.** For any trigger that requires a `symptom` field with a quoted prior turn or quoted artifact text, log nothing if the quote isn't available. Better to miss the entry than poison `/vibe-wrap-evolve` with an unverifiable claim.
...
4. **Pattern #11 namespace isolation.** Every entry lands in `~/.gemini/antigravity/data/vibe-wrap/friction.jsonl`. Never any other namespace, never any sibling's friction file.

## Universal triggers (any vibe-wrap command)

| Trigger | Friction type | Confidence | Notes |
|---------|---------------|------------|-------|
| Sentinel session-log entry has no terminal pair after 24h (detected by `vibe-wrap-friction-logger.detect_orphans()` at next command start) | `command_abandoned` | high | Emitted out-of-band by the next vibe-wrap command's startup check. Per-command sections do **not** call this. |
...
## /vibe-wrap (the router / main command)

The load-bearing user-facing command. Highest interaction surface in the plugin — most friction lives here.

| Trigger | Friction type | Confidence | Notes |
|---------|---------------|------------|-------|
| User declines a Pattern #13 complement offer at workflow start (e.g., vibe-wrap announces it would defer to a present sibling for a sub-step and the user picks `[skip]`) | `complement_rejected` | high | Set `complement_involved` to the complement name (e.g., `vibe-cartographer:reflect`). |
...
| User rewrites >50% of the generated wrap doc at `docs/session-wraps/<ts>.md` post-write (measured by line diff at the next `/vibe-wrap-evolve` run) | `artifact_rewritten` | medium | Wrap docs are personal and edits are common — confidence is medium, not high. Measure at evolve time, not in-line. |
...
| User declines the dashboard bridge gate (`n` at the bridge prompt) when the threshold fired | `complement_rejected` | high | Set `complement_involved` to `mcp__626labs-cloud__bridge_context_to_architect`. Strong signal — threshold logic may be too loose, or bridge prompt copy doesn't communicate stakes well. |
...
| Trigger | Friction type | Confidence | Notes |
|---------|---------------|------------|-------|
| (no command-specific triggers) | — | — | `/vibe-wrap-status` is read-only — no gates, no defaults to override, no artifacts to rewrite, no complements to decline. Friction surfaces only via the universal triggers (`repeat_question`, `rephrase_requested`) above. |

> **Forward-looking note:** if a future version of `/vibe-wrap-status` grows interactive behavior (e.g., asks the user to choose what to display), revisit this section.

---

## vibe-wrap-plant (skill — internal)

Internal skill — invoked by sibling plugins, not by the user directly. Friction comes from siblings' contracts, not from the plant skill's own interaction surface.
...
> Note: `vibe-wrap-plant` doesn't have its own user-invocable command path, so most vibe-wrap-friction-logger discipline applies via the calling sibling's SKILL, not via `/plant` itself.
...
4. Add the matching `vibe-wrap-friction-logger.log()` invocation in the command SKILL at the trigger point. The bidirectional consistency (this map ↔ actual call sites) is audited by `evolve-wrap` over time.
...
- Cart's friction-triggers reference — `~/.gemini/antigravity/plugins/cache/vibe-plugins/vibe-cartographer/<ver>/skills/vibe-wrap-guide/references/friction-triggers.md`. vibe-wrap mirrors the structural shape; the per-command rows are vibe-wrap-specific.
