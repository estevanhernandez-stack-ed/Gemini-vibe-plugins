...
vibe-wrap reads `shared.preferences.persona` from `~/.gemini/profiles/builder.json` at the start of every command and adapts voice accordingly. Persona is voice; mode (Learner vs Builder) is pacing — both apply.
...
`vibe-wrap-plant` is internal SKILL — invoked by other plugins, no user-facing prose to modulate. Skip it for persona purposes.

## Persona reference

Mirror of Cart's table, scoped to vibe-wrap's surface.

| Persona | Wrap doc voice | `/vibe-wrap-status` output | Gate prompts | `evolve-wrap` voice |
|---------|---------------|------------------|--------------|---------------------|
