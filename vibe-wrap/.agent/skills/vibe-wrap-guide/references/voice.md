...
- **A reference doc that's specifically about reference voices** (this file, the persona-adaptation reference, the broader CODER VOICE SYNTHESIS in the user's `~/.gemini/antigravity/AGENTS.md`). Quoting examples that violate the rules is fine because it's quoting, not authoring.
...
Pulled from:
- `vibe-plugins/AGENTS.md` § Tech Stack & Voice (marketplace voice rules).
- `~/.gemini/antigravity/AGENTS.md` § CODER VOICE SYNTHESIS (the broader Architect voice frame, which vibe-wrap inherits).
