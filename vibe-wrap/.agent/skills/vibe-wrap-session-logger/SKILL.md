---
name: vibe-wrap-session-logger
description: "Internal SKILL — not a user workflow. Two-phase append-only session log for vibe-wrap's OWN sessions: a sentinel entry at command start (`outcome=in_progress`) and a terminal entry at command end, paired by sessionUUID. Distinct from breadcrumb capture — this is vibe-wrap's self-evolution instrumentation (Pattern #2), not the cross-plugin trail. Storage: `~/.gemini/antigravity/data/vibe-wrap/sessions/<YYYY-MM-DD>.jsonl`. Pattern #11 namespace isolation — writes ONLY inside vibe-wrap's data dir. Invoked by every vibe-wrap command at start and end."
---

# vibe-wrap-session-logger — Sentinel + Terminal Session Log

Internal SKILL. Not a user-invocable slash command. Every vibe-wrap command calls `start()` at invocation and the terminal-append procedure at completion. The two entries share a `sessionUUID` so future tooling (`/vibe-wrap-evolve`, an eventual `vibe-wrap-friction-logger.detect_orphans()`) can pair them.

Read `AGENTS.md` + the `vibe-wrap-guide` skill (`.agent/skills/vibe-wrap-guide/SKILL.md`) first for shared behavior — voice, namespace isolation, atomic-append discipline.

## Before You Start

- **Atomic protocol:** all session log writes go through `python .agent/scripts/atomic-append-jsonl.py`. Never `>>` from a shell.
...
- **Pattern #11 namespace isolation.** This SKILL writes to exactly one place: `~/.gemini/antigravity/data/vibe-wrap/sessions/<YYYY-MM-DD>.jsonl`. Never `shared.*`, never any sibling namespace, never `~/.gemini/profiles/builder.json`. Cart's vibe-wrap-session-logger updates `last_seen_complements` on the unified profile; vibe-wrap v0.1.0 deliberately does NOT — that snapshot is deferred to `/vibe-wrap-evolve`.
...
`~/.gemini/antigravity/data/vibe-wrap/sessions/<YYYY-MM-DD>.jsonl`
...
- **plugin_version** — read from `.agent/agent.json`. If unreadable, use `"unknown"`.
- **command** — which vibe-wrap command is running: `wrap`, `status`, `vibe-wrap-plant`, `evolve-wrap`. (Internal SKILLs `vibe-wrap-guide`, `vibe-wrap-session-logger`, `vibe-wrap-friction-logger` don't write their own session entries — they're invoked from commands that already wrote one.)
- **project_dir** — basename of the current working directory. Not the full path. PII discipline.
- **mode** — `learner` | `builder` | `null`. Read from `~/.gemini/profiles/builder.json` at `shared.preferences.mode`.
...
- **friction_notes** — array of short strings. Human-facing recap. The actual structured friction signal goes to `friction.jsonl` via `vibe-wrap-friction-logger.log()`.
...
**Arguments:**
- `command` — the command name (`wrap`, `status`, `vibe-wrap-plant`, `evolve-wrap`).
...
2. **Determine audit fields.**
   ...
   - `plugin: "vibe-wrap"`.
   - `plugin_version`: read from `.agent/agent.json`'s `"version"`. Fall back to `"unknown"`.
   - `mode` / `persona`: read from `~/.gemini/profiles/builder.json`. Pass through as-is; `null` if unset or file missing.
3. **Build the sentinel entry** using the shape above with `outcome: "in_progress"`.
4. **Atomic append.** Pipe the JSON entry to:
   ```
   python .agent/scripts/atomic-append-jsonl.py ~/.gemini/antigravity/data/vibe-wrap/sessions/<today>.jsonl
   ```
...
**Failure handling:** same as `start()` — a failed append logs a one-line warning to stderr and the command proceeds. The user never sees a vibe-wrap-session-logger error.

## Namespace Isolation (Pattern #11)

This SKILL writes to exactly one place:

`~/.gemini/antigravity/data/vibe-wrap/sessions/<date>.jsonl` — session log file (vibe-wrap's own data directory, append-only).

It does NOT write to:
- `~/.gemini/profiles/builder.json` — Cart's vibe-wrap-session-logger updates `plugins.vibe-cartographer._meta.last_seen_complements` here; vibe-wrap v0.1.0 does not. The `last_seen_complements` snapshot is deferred to `/vibe-wrap-evolve`. When that lands, the analogous block under `plugins.vibe-wrap._meta.last_seen_complements` will be the only profile field vibe-wrap touches — never `shared.*`, never any other plugin's namespace.
...
1. The session JSONL stays empty for that run. `/vibe-wrap-evolve` will see no entry — friction signals from that run are recoverable only from the chat transcript or process-notes.
...
- User-inspectable. `cat ~/.gemini/antigravity/data/vibe-wrap/sessions/<date>.jsonl` is the read surface in v0.1.0; later versions may ship a friendlier viewer.
...
The session log is raw material for **Level 3** of the Self-Evolving Plugin Framework. `/vibe-wrap-evolve` reads these entries (alongside `friction.jsonl` and the wrap docs themselves) to propose plugin improvements based on observed patterns.
...
See `AGENTS.md` + the `vibe-wrap-guide` skill (`.agent/skills/vibe-wrap-guide/SKILL.md`) for the broader Pattern #11 / Pattern #13 contract and the Self-Evolving Plugin Framework reference.

## Wiring (forward note)

Every future vibe-wrap command SKILL (`wrap`, `status`, `vibe-wrap-plant`, `evolve-wrap`) calls `start.py` at workflow start to mint a sessionUUID, holds the UUID in memory for the duration of the run, passes it to `vibe-wrap-friction-logger.log()` for any friction events, and calls `end.py` at workflow end with the partial terminal entry. SKILL bodies that get implemented in later checklist items will wire this in. Item 3 builds the infrastructure; the wire-up is per-command.
...
- [`../vibe-wrap-guide/references/friction-triggers.md`](../vibe-wrap-guide/references/friction-triggers.md) — per-command friction-trigger contract that `vibe-wrap-friction-logger.log()` honors.
