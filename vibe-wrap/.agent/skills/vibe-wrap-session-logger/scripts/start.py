...
file at `~/.gemini/antigravity/data/vibe-wrap/sessions/<YYYY-MM-DD>.jsonl`.
...
Notes:
    ...
    - mode and persona are read from ~/.gemini/profiles/builder.json if
...
# Antigravity port: vibe-wrap-session-logger/scripts -> vibe-wrap-session-logger
# -> skills -> .agent (the port plugin root). agent.json holds the mirrored
# version; atomic-append lives in .agent/scripts/.
PLUGIN_ROOT = SCRIPT_DIR.parent.parent.parent
PLUGIN_MANIFEST = PLUGIN_ROOT / "agent.json"
ATOMIC_APPEND = PLUGIN_ROOT / "scripts" / "atomic-append-jsonl.py"

DATA_DIR = Path.home() / ".gemini" / "antigravity" / "data" / PLUGIN_NAME / "sessions"
PROFILE_PATH = Path.home() / ".gemini" / "profiles" / "builder.json"
