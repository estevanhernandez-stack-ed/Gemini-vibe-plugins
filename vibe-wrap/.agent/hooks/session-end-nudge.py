...
Fires once at session close (the `SessionEnd` lifecycle event — Claude Code
and Antigravity/Gemini CLI share this event name plus the stdin-JSON contract).
Checks three cheap signals; if any is high, emits a single one-line nudge to
stdout:
...
Payload: the SessionEnd hook receives JSON on stdin with fields:
    session_id        — UUID of the closing session.
    transcript_path   — path to the session transcript JSONL.
    cwd               — current working directory.
    hook_event_name   — "SessionEnd".
    reason            — one of exit / clear / logout / prompt_input_exit /
                        other. (Antigravity/Gemini name; the Claude Code
                        original called this field `why`. This script reads
                        neither — it uses only session_id + cwd — so the
                        rename is non-breaking. Documented for fidelity.)
...
BREADCRUMBS_DIR = (
    Path.home() / ".gemini" / "antigravity" / "data" / "vibe-wrap" / "breadcrumbs"
)
