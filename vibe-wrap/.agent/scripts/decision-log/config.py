...
Smart-default file path resolver:
    ...
    - Else: ~/.gemini/antigravity/decisions.md  (or .jsonl)
...
GLOBAL_CONFIG_PATH = Path.home() / ".gemini" / "antigravity" / "data" / "vibe-wrap" / "config.json"
...
def smart_default_path(extension: str = "md", cwd: Path | None = None) -> Path:
    ...
    Returns:
        ...
        ~/.gemini/antigravity/decisions.<ext> as the user-scoped fallback.
    ...
    return Path.home() / ".gemini" / "antigravity" / f"decisions.{extension}"
