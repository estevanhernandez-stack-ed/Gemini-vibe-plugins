...
- Else → `~/.gemini/antigravity/decisions.md` (user-scoped, lands next to cloud-syncable personal files — Dropbox, iCloud, OneDrive territory).
...
The MCP backend respects the project-binding convention from `~/.gemini/antigravity/AGENTS.md`. If the current repo is bound to a 626Labs project, the decision tags with that project's ID. If unbound, `projectId: null` and the description includes the repo name as a fallback tag (matching the existing decision-logging discipline).
...
2. **Global config** — `~/.gemini/antigravity/data/vibe-wrap/config.json`.
...
```json
{
  ...
  "decision_log": {
    "backend": "file-md",
    "file_path": "~/.gemini/antigravity/decisions.md",
...
later by re-running with --reconfigure or editing the config file.
  ...
  1. Markdown file at <smart-default>/decisions.md   ← recommended
     human-readable, lives in your repo (or ~/.gemini/antigravity/), easy to share
...
- **Subsystem layout** — `spec.md > Decision-log subsystem` lists the seven Python modules under `.agent/scripts/decision-log/`.
