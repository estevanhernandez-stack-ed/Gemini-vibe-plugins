...
Persona, posture, the honest-coverage stance, and hard rules are always-on via `AGENTS.md`. Reference detail (data contracts, friction-trigger map, the anchored complement registry, schemas) lives in the `vibe-test-guide` skill (`.agent/skills/vibe-test-guide/SKILL.md`) — load it to ground any proposed contract, trigger, or schema change. Then execute this workflow.
...
- **Blocking:** at least one session-log entry must exist under `~/.gemini/antigravity/data/vibe-test/sessions/`. If not: *"You haven't run any Vibe Test command yet. Run `/vibe-test` then `/vibe-test-audit` on a real project first, then come back."*
...
- **Data contracts:** [`.agent/skills/vibe-test-guide/references/data-contracts.md`](.agent/skills/vibe-test-guide/references/data-contracts.md) — friction + wins + session-log shapes. Read every file listed there.
- **Friction triggers:** [`.agent/skills/vibe-test-guide/references/friction-triggers.md`](.agent/skills/vibe-test-guide/references/friction-triggers.md) — per-command trigger map. Use this to decide what friction counts as signal for which command.
- **Plays-well-with:** [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md) — anchored complements. Pattern-13 proposals against this map get extra scrutiny (they're universal commitments).
- **Vitals (pre-flight):** load the `vibe-test-vitals` skill (`.agent/skills/vibe-test-vitals/SKILL.md`) — invoke its seven read-only checks at the top as a structural pre-flight. Vitals is an **internal skill**, not a slash workflow in v0.2: it runs only from here. If vitals reports ✗ failures, surface them in a "house is not clean" banner and ask the builder whether to proceed anyway. The default is **abort**; clean house first.
- **Workflow/skill files you can propose edits to:** the workflow + skill bodies under `.agent/workflows/*.md` and `.agent/skills/**/SKILL.md` (see the self-edit target map in the next section). Never propose edits to `docs/`, `tests/`, or `src/`.

## Session Logging

At workflow start, call `vibe-test-session-logger.start("evolve", <repo-root>)` (the vibe-plugins root, resolved from the SKILL's own file location). Hold the sessionUUID for the duration of this command. Pass it to any `vibe-test-friction-logger.log()` invocation (evolve itself does not emit friction per `friction-triggers.md`; proposal rejections become `default_overridden` against the **target** command — see Step 5b).

At workflow end, call `vibe-test-session-logger.end({ sessionUUID, command: "evolve", outcome })`:
...
Load and run the `vibe-test-vitals` skill (`.agent/skills/vibe-test-vitals/SKILL.md`) as a read-only structural check. It is an internal skill — not a slash workflow — invoked only here. Render its banner in-line. If vitals reports any ✗ **fail**, show:

```
  House is not clean — vitals found <N> fail(s).
  /vibe-test-evolve works best against a consistent install. Fix vitals first, then retry.

  [Proceed anyway]  [Abort]
```

Default to `[Abort]`. Proceeding is only for advanced use (builder knows the failure is harmless, e.g., a missing `~/.gemini/antigravity/data/vibe-test/` because they haven't run commands yet).

### 2. Read inputs

Read the following over the last **30 days**, line-by-line, silently skipping malformed lines:

1. `~/.gemini/antigravity/data/vibe-test/friction.jsonl` — friction entries.
2. `~/.gemini/antigravity/data/vibe-test/wins.jsonl` — win entries (Pattern #14 baseline).
3. `~/.gemini/antigravity/data/vibe-test/sessions/*.jsonl` — session logs.

Then read every workflow + skill body under `.agent/workflows/*.md` and `.agent/skills/**/SKILL.md` so your proposed diffs quote exact current text.
...
**Self-edit target map (name the real port target — this is NOT a blind path).** A source surface that became a workflow lives in a different dir than one that stayed a skill. Map each proposed edit to its port target:

| What you're proposing to change | Port target |
|---|---|
| `audit` / `coverage` / `fix` / `gate` / `generate` / `posture` command behavior | `.agent/workflows/vibe-test-<cmd>.md` (each merged the command + skill into one workflow) |
| Router behavior (bare `/vibe-test`) | `.agent/workflows/vibe-test.md` |
| evolve behavior | `.agent/workflows/vibe-test-evolve.md` (this file) |
| Persona / posture / honest-coverage stance / hard rules (always-on) | `AGENTS.md` |
| A logger's behavior | `.agent/skills/vibe-test-session-logger/SKILL.md`, `.agent/skills/vibe-test-friction-logger/SKILL.md`, or `.agent/skills/vibe-test-wins-logger/SKILL.md` |
| Decay behavior | `.agent/skills/vibe-test-decay/SKILL.md` |
| Vitals checks (now an internal skill, not a workflow) | `.agent/skills/vibe-test-vitals/SKILL.md` |
| Guide reference detail (schemas, CLI patterns, registry) | `.agent/skills/vibe-test-guide/SKILL.md` or its `references/` / `schemas/` files |
| Data contracts / friction-trigger map / anchored `plays-well-with.md` | `.agent/skills/vibe-test-guide/references/*.md` (cross-command evidence required — load-bearing invariants) |
| Classifier / scoring / state-schema / generator **code** | the CLI source repo (`src/**/*.ts`) — **NOT** ported into `.agent/`. Those proposals ship on the next CLI publish, not as a `.agent/` edit. |
...
```json
{
  ...
  "proposed_skill_edit": {
    "file": "<port target from the self-edit map — e.g. .agent/workflows/vibe-test-<command>.md, .agent/skills/vibe-test-<skill>/SKILL.md, or AGENTS.md>",
...
- **Never edit `docs/`, `tests/`, `src/`.** `/vibe-test-evolve` operates on SKILL prose only. Code changes require a human-designed PR.
...
- **Never touch the user's app.** `/vibe-test-evolve` reads logs about Vibe Test usage; it never scans, classifies, or rewrites the user's code.
...
No handoff. `/vibe-test-evolve` is a standalone reflection run. Builder reviews `proposed-changes.md` when ready.
