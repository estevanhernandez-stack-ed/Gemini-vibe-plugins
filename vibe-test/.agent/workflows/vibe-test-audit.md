...
Persona, experience-level adaptation, the F2 honest-coverage stance, the Pattern #13 composition posture, and version resolution are always-on via `AGENTS.md`. The full classification matrix (6 app types × 5 tiers × modifiers), the data contracts, and the audit-state schema live in the `vibe-test-guide` skill (`.agent/skills/vibe-test-guide/SKILL.md`) — load it before you classify and before you validate the `audit.json` / `covered-surfaces.json` writes. Then follow this workflow end to end.
...
- **Data contract** — [`.agent/skills/vibe-test-guide/references/data-contracts.md`](.agent/skills/vibe-test-guide/references/data-contracts.md) section "audit-state". You own writes to `project-state.ts` + `audit.json` + `audit-<hash>.json` + `covered-surfaces.json`. You read `findings.jsonl` from vibe-sec when present.
- **Plays well with** — [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md). Entries with `applies_to: audit` are `vibe-doc`, `vibe-sec`, and `playwright` (for UI-heavy repos).
- **Friction triggers** — [`.agent/skills/vibe-test-guide/references/friction-triggers.md`](.agent/skills/vibe-test-guide/references/friction-triggers.md) `/vibe-test-audit` section. Hook `vibe-test-friction-logger.log()` at the six trigger points listed there.
- **Classification matrix** — [guide > "Classification Matrix"](.agent/skills/vibe-test-guide/SKILL.md#classification-matrix). This is the single source of truth for app types + tiers + modifiers.
...
- **Session logger** — the `vibe-test-session-logger` skill (`.agent/skills/vibe-test-session-logger/SKILL.md`). `start('audit', project)` at entry, `end({sessionUUID, command: 'audit', outcome})` at exit. Same sessionUUID threads through friction + wins + beacon writes during the run.
...
2. Invoke `vibe-test-session-logger.start('audit', project_basename)`. Hold the returned `sessionUUID` in memory until Step 11.
3. Compose the persona-adapted opening line per `AGENTS.md § Persona`.

### Step 1 — Blocking prereq check

Run the blocking prereq described above. If the check fails, render a gentle block and halt. Do NOT proceed with any scanning or state writes; do invoke `vibe-test-session-logger.end({outcome: 'aborted'})`.

### Step 2 — Pattern #13 announcement (anchored complements)

Parse [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md) via `src/composition/anchored-registry.ts` → `loadAnchoredRegistry()`. Filter to entries whose `applies_to` includes `audit`. For each entry present in the workflows/skills available in this Antigravity workspace (cross-checked via `src/composition/detect-complements.ts`), include a `Deferral` record in the `ReportObject` with the verbatim `deferral_contract`. The final banner renders a "Plays well with" section when any deferrals are present.
...
2. **`vibe-test-session-logger.end({sessionUUID, command: 'audit', outcome: 'completed', key_decisions, complements_invoked, artifact_generated})`** — terminal entry paired to the Step 0 sentinel.
...
**Do NOT prescribe `/clear` between commands.** the host auto-compacts; this is a user-tested correction (see `memory/feedback_no_manual_clear.md`).
...
Hook `vibe-test-friction-logger.log({sessionUUID, friction_type, symptom, confidence, ...})` at these trigger points (per [friction-triggers.md](.agent/skills/vibe-test-guide/references/friction-triggers.md) audit section):
...
When in doubt, don't log. The vibe-test-friction-logger has a quoted-prior gate for question-type triggers — if you're about to log `repeat_question` / `rephrase_requested`, the `symptom` must contain a quoted snippet of the prior turn.
