...
Persona, experience-level adaptation, posture, and the Pattern #13 composition rules are always-on via `AGENTS.md`. The data contracts and the fix/pending-dir schemas live in the `vibe-test-guide` skill (`.agent/skills/vibe-test-guide/SKILL.md`) — load it before you stage a repair or validate the `fix.json` / `accepted.json` / `rejected.json` writes. Then follow this workflow end to end.
...
- **Plays well with** — [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md). Entries with `applies_to: fix` are `superpowers:systematic-debugging` (flagship deferral) and `superpowers:verification-before-completion` (post-repair verification co-invoke).
- **Friction triggers** — [`.agent/skills/vibe-test-guide/references/friction-triggers.md`](.agent/skills/vibe-test-guide/references/friction-triggers.md) `/vibe-test-fix` section.
- **Tier-adaptive language** — `src/reporter/tier-adaptive-language.ts`.
- **Session logger** — the `vibe-test-session-logger` skill (`.agent/skills/vibe-test-session-logger/SKILL.md`). `start('fix', project)` at entry, `end({sessionUUID, command: 'fix', outcome})` at exit.
- **Friction logger** — the `vibe-test-friction-logger` skill (`.agent/skills/vibe-test-friction-logger/SKILL.md`). Entries when a rollback happens (signals that auto-write confidence mis-calibrated) or when a complex diagnosis is deferred.
...
2. Invoke `vibe-test-session-logger.start('fix', project_basename)`. Hold the returned `sessionUUID` in memory until Step 9.
3. Compose the persona-adapted opening line per `AGENTS.md § Persona`.

### Step 1 — Blocking prereq check

Run the blocking prereq described above. If the check fails, render a gentle block and halt. Do NOT proceed; do invoke `vibe-test-session-logger.end({outcome: 'aborted'})`.

### Step 2 — Pattern #13 announcement (anchored complements)

Parse [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md) via `loadAnchoredRegistry()`. Filter to entries whose `applies_to` includes `fix`. For each entry present in the workflows/skills available in this Antigravity workspace:
...
Log `friction_type: "complex_diagnosis_deferred"` at confidence `medium` whenever a failure takes the defer path — the aggregate signal tells /vibe-test-evolve where fix's automatic diagnosis is weakest.
...
1. **`vibe-test-session-logger.end({sessionUUID, command: 'fix', outcome: 'completed', key_decisions, complements_invoked, artifact_generated})`** — terminal entry paired to Step 0.
...
**Do NOT prescribe `/clear` between commands.** the host auto-compacts.
