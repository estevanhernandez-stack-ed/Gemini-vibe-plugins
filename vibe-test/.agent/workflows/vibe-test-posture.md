...
Persona, experience-level adaptation, and posture are always-on via `AGENTS.md`. The data contracts (posture reads every state file, writes only its own sidecar + the session log) live in the `vibe-test-guide` skill (`.agent/skills/vibe-test-guide/SKILL.md`) — load it to know which state files to summarize. Then follow this workflow end to end.
...
- **Data contract** — [`.agent/skills/vibe-test-guide/references/data-contracts.md`](.agent/skills/vibe-test-guide/references/data-contracts.md). Posture reads ALL state files but writes NONE except the session-log entry and the JSON sidecar `.vibe-test/state/posture.json`.
- **Plays well with** — [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md). No entries specifically tagged `applies_to: posture` — posture is a scan-less summary, not a deferral surface. Teaser mentions allowed but not announcements.
- **Friction triggers** — [`.agent/skills/vibe-test-guide/references/friction-triggers.md`](.agent/skills/vibe-test-guide/references/friction-triggers.md) `/vibe-test-posture` section.
- **Tier-adaptive language** — `src/reporter/tier-adaptive-language.ts`.
- **Session logger** — the `vibe-test-session-logger` skill (`.agent/skills/vibe-test-session-logger/SKILL.md`). `start('posture', project)` at entry, `end({sessionUUID, command: 'posture', outcome})` at exit.
...
2. Invoke `vibe-test-session-logger.start('posture', project_basename)`. Hold the returned `sessionUUID` in memory until Step 6.
3. Compose the persona-adapted opening line per `AGENTS.md § Persona`. Keep it under 2 lines.
...
1. **`vibe-test-session-logger.end({sessionUUID, command: 'posture', outcome: 'completed', key_decisions: [<next-action suggestion chosen>, <staleness classifier>], complements_invoked: [], artifact_generated: '<path to JSON sidecar>'})`** — terminal entry paired to Step 0.
...
Posture is a low-friction SKILL. Most signals come from `/vibe-test-audit`, `/vibe-test-generate`, `/vibe-test-fix`, `/vibe-test-gate`. When in doubt, don't log.
