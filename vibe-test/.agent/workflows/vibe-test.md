---
description: "Router — identity, state-awareness, next-step prompt. The bare `/vibe-test` entry point that introduces the plugin and routes you to your next action. Greets the builder, detects first-run vs returning state, lists the 6 subcommands in plain language, asks in natural language what to do next. No scanning. Under 10 seconds on first render."
---

# /vibe-test — Bare Entry Point (Router)

Persona, experience-level language adaptation, Pattern #13 composition rules, and version-resolution policy are always-on via `AGENTS.md`. Deep reference (schemas, the anchored registry, friction-trigger map) lives in the `vibe-test-guide` skill (`.agent/skills/vibe-test-guide/SKILL.md`) — load it when you need to validate a complement lookup or a state read. Then follow this workflow end-to-end.
...
- **Always-on layer** — `AGENTS.md` carries persona, experience-level adaptation, Pattern #13 composition posture, version resolution, and prereq shaping. Every behavior rule flows through it.
- **Version resolution rules** — `AGENTS.md § Version Resolution (Pattern #15)`.
- **Prereq shaping rules** — `AGENTS.md § Prereq Shaping (Pattern #16)`. The returning-vs-first-run detection here is a **shaping** prereq, not a blocking one.
- **Pattern #13 anchored registry** — [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md). The router only announces complements whose `applies_to` array includes `router` or whose `phase` is explicitly entry-level. Most anchored complements apply to commands like `audit` / `generate` and are deferred to those commands — the router does NOT pre-announce them.
- **Session logger** — the `vibe-test-session-logger` skill (`.agent/skills/vibe-test-session-logger/SKILL.md`). Two-phase: `start(command='router', project)` at entry, `end({sessionUUID, command:'router', outcome})` before handoff.
- **Builder profile** — `~/.gemini/profiles/builder.json`. Read `shared.preferences.persona`, `shared.technical_experience.level`, and `plugins.vibe-test.testing_experience` if present. Defaults documented below.

## Step 1 — Resolve Plugin Version (Pattern #15)

Read `~/.gemini/antigravity/plugins/installed_plugins.json` and look for the `vibe-test` entry. Expected shape varies slightly by host version, but every shape exposes a `version` field and/or a path that contains the installed version. Use the version from this file if present.
...
2. If `active-path.json` is missing or unreadable, fall back to `.agent/agent.json`'s `version` field.
...
Before any user-facing output, invoke vibe-test-session-logger `start`:
...
Hold the returned `sessionUUID` in memory for the rest of the run. Follow the `vibe-test-session-logger` skill (`.agent/skills/vibe-test-session-logger/SKILL.md`) for the exact invocation pattern.

## Step 3 — Read Builder Profile (for persona + experience adaptation)

Read `~/.gemini/profiles/builder.json`. Three fields matter to the router:
...
Apply the persona to the **opening line** only (per `AGENTS.md § Persona Adaptation`). The banner body stays neutral so identity + command list render consistently across personas. Experience level adapts the body verbosity per `AGENTS.md § Experience-Level Language Adaptation (SE7)`.
...
At the router level, the anchored registry is **mostly not relevant** — the 7 complements in [`plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md) all apply to `audit` / `generate` / `fix` / `coverage` / `gate`, not to `router`. You do not pre-announce those deferrals on the bare `/vibe-test` screen.

However, do the following so the banner can hint at composition when it matters:

1. Read the list of `vibe-*` workflows/skills available in this Antigravity workspace.
...
End with the persona-adapted handoff line per `AGENTS.md § Handoff Language Rules`. For the router the handoff line IS the next-step question — you don't need a separate "run `/<next>` when ready" closing. The question embeds the handoff naturally.

**Do NOT prescribe `/clear` between commands.** The host auto-compacts; the obsolete `/clear, then run` pattern is explicitly disallowed.

## Step 8 — Session Logger: Terminal Entry

After the banner renders and before returning control to the user, invoke vibe-test-session-logger `end`:
...
```
{
  ...
  key_decisions: [
    ...
    <optional: "version resolved via installed_plugins.json" | "via active-path.json" | "via agent.json" | "unresolved">,
  ],
...
- No file scanning beyond 4 small JSON reads (`installed_plugins.json`, `active-path.json`, `agent.json`, optionally `.vibe-test/state.json`).
- No AST parsing.
- No network calls.
- No coverage runs.
- Session-log writes are append-only and non-blocking.

If you find yourself about to do a scan, classification, or coverage call from the router — stop. Those live in the other workflows.

## What the Router is NOT

- Not a classifier. Classification lives in the `/vibe-test-audit` workflow.
- Not a generator. Generation lives in the `/vibe-test-generate` workflow.
...
The router is a friction-light surface. The one documented trigger:

- **Abandoned-router detection** — if a sentinel for `command='router'` exists in today's sessions file with no paired terminal entry and the sentinel is >24h old, vibe-test-friction-logger's `detect_orphans()` pairs it as `friction_type: "command_abandoned"`. Router invokes `detect_orphans()` once at first-run-of-the-day. Full contract in [`.agent/skills/vibe-test-guide/references/friction-triggers.md`](.agent/skills/vibe-test-guide/references/friction-triggers.md).
