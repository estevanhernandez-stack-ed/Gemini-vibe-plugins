...
Persona, experience-level adaptation, the F2 honest-coverage stance, and the Pattern #13 composition posture are always-on via `AGENTS.md`. The data contracts and the coverage-state schema live in the `vibe-test-guide` skill (`.agent/skills/vibe-test-guide/SKILL.md`) — load it before you validate the `coverage.json` write. Then follow this workflow end to end.
...
- **Data contract** — [`.agent/skills/vibe-test-guide/references/data-contracts.md`](.agent/skills/vibe-test-guide/references/data-contracts.md) section "coverage-state". You own writes to `<repo>/.vibe-test/state/coverage.json` (and scoped variant) + its history copy. You read `audit.json` for source-file list when present.
- **Plays well with** — [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md). Entries with `applies_to: coverage` are `tessl:analyzing-test-coverage` (raw parsing deferral).
- **Friction triggers** — [`.agent/skills/vibe-test-guide/references/friction-triggers.md`](.agent/skills/vibe-test-guide/references/friction-triggers.md) `/vibe-test-coverage` section.
- **Tier-adaptive language** — `src/reporter/tier-adaptive-language.ts`.
- **Session logger** — the `vibe-test-session-logger` skill (`.agent/skills/vibe-test-session-logger/SKILL.md`). `start('coverage', project)` at entry, `end({sessionUUID, command: 'coverage', outcome})` at exit.
...
2. Invoke `vibe-test-session-logger.start('coverage', project_basename)`. Hold the returned `sessionUUID` in memory until Step 8.
...
Parse [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md) via `loadAnchoredRegistry()`. Filter to entries whose `applies_to` includes `coverage`.
...
1. **`vibe-test-session-logger.end({sessionUUID, command: 'coverage', outcome: 'completed', key_decisions: [<adapter accepted/declined>, <denominator honest flag>], complements_invoked, artifact_generated})`** — terminal entry paired to Step 0.
