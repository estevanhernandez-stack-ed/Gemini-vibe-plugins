...
Persona, experience-level adaptation, posture, and the Pattern #13 composition rules are always-on via `AGENTS.md`. The data contracts and the generate-state schema live in the `vibe-test-guide` skill (`.agent/skills/vibe-test-guide/SKILL.md`) — load it before you write tests to `tests/` or `.vibe-test/pending/` and validate the generate-state sidecar. Then follow this workflow end to end.
...
- **Guide SKILL** — `AGENTS.md` + the `vibe-test-guide` skill (`.agent/skills/vibe-test-guide/SKILL.md`). Persona opening/handoff lines, experience-level verbosity, Pattern #13 anchored + dynamic, session memory interfaces.
- **Data contract** — [`.agent/skills/vibe-test-guide/references/data-contracts.md`](.agent/skills/vibe-test-guide/references/data-contracts.md) section "generate-state". You own writes to the generated tests themselves (to `tests/` or `.vibe-test/pending/`), `.vibe-test/pending/index.md`, and the generate-state JSON sidecar. You read `audit.json` (or scoped variant), `accepted.json` / `rejected.json`, and any pre-existing pending-dir contents.
- **Plays well with** — [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md). Entries with `applies_to: generate` are `superpowers:test-driven-development`, `playwright`, `vibe-doc`, `vibe-sec`.
- **Friction triggers** — [`.agent/skills/vibe-test-guide/references/friction-triggers.md`](.agent/skills/vibe-test-guide/references/friction-triggers.md) `/vibe-test-generate` section.
...
- **Session logger** — the `vibe-test-session-logger` skill (`.agent/skills/vibe-test-session-logger/SKILL.md`). `start('generate', project)` at entry, `end({sessionUUID, command: 'generate', outcome})` at exit.
- **Friction logger** — the `vibe-test-friction-logger` skill (`.agent/skills/vibe-test-friction-logger/SKILL.md`). Per-reject entries when builders supply rejection reasons. The rejection-*pattern* probe (3-consecutive-rejects) is item #8; this command captures each reject individually.
...
1. Read `shared.preferences.persona`, `shared.preferences.pacing`, and `plugins.vibe-test.testing_experience` (fallback `shared.technical_experience.level`) from `~/.gemini/profiles/builder.json`.
2. Invoke `vibe-test-session-logger.start('generate', project_basename)`. Hold the returned `sessionUUID` until Step 10.
3. Compose the persona-adapted opening line per `AGENTS.md § Persona`.
...
Parse [`.agent/skills/vibe-test-guide/references/plays-well-with.md`](.agent/skills/vibe-test-guide/references/plays-well-with.md) via `loadAnchoredRegistry()`. Filter to entries whose `applies_to` includes `generate`. For each entry present in the workflows/skills available in this Antigravity workspace (cross-checked via `detectComplements()`), surface the verbatim `deferral_contract`.
...
6. On force-accept: re-call `acceptPendingTest({..., force: true})`. Log `friction_type: "artifact_rewritten"` at confidence `low` — the builder overrode a safety check; worth noticing in `/vibe-test-evolve`.
...
3. **`vibe-test-session-logger.end({sessionUUID, command: 'generate', outcome: 'completed', tests_generated, tests_accepted, tests_rejected, rejection_reasons, levels_covered, framework_used, complements_invoked, artifact_generated: '<path to markdown>'})`** — terminal entry.
...
Persona-adapted handoff line per `AGENTS.md § Handoff language rules`:
...
**Do NOT prescribe `/clear` between commands.** the host auto-compacts; this is a user-tested correction.
...
Hook `vibe-test-friction-logger.log({sessionUUID, friction_type, symptom, confidence, ...})` at these trigger points:
...
When in doubt, don't log. False positives poison `/vibe-test-evolve`.
...
The base flow above is the happy path. Three safety features extend it: dry-run preview (G9), rejection-pattern probe (G4), and L2 feedback capture (feeds `/vibe-test-evolve`). These are *additive* — they do not change Steps 0–9 when the builder runs the command without `--dry-run` and without hitting a probe trigger.
...
3. **Session-log policy during dry-run.** The vibe-test-session-logger *sentinel* at Step 0 still fires (so the dry-run itself is discoverable in L2). All *other* session-log writes (accept/reject events, probe-fired marker, terminal entry) are deferred-with-tag: if the SKILL writes them at all, include `context.dry_run: true` so `/vibe-test-evolve` filters them out of L2 aggregation. Default policy: write only the sentinel + terminal entries (both tagged `dry_run: true`); skip per-test accept/reject events entirely — dry-run is a preview, not real feedback.
...
2. If `true`:
   ...
   4. Read the builder's response. Branch on **SKILL reasoning**:
      ...
      When in doubt between the two branches, prefer the friction branch. False-positive friction poisons `/vibe-test-evolve` less than false-positive wins (which would let a real problem sit unfixed).
...
**Every accept and every reject writes a session-log entry.** This is *additive* to Steps 3f, 4, and 5 — the state writes in Step 8 still happen. The feedback entries feed `/vibe-test-evolve` at Level 2 (session memory); without them, `/vibe-test-evolve` can see counts but not individual decision context.
...
**Dry-run guard:** during `--dry-run`, **do not** call `recordFeedbackEvent` — preview mode has no real decisions. If for some reason the SKILL decides to write during dry-run, pass `dryRun: true` to tag the entry; `/vibe-test-evolve` filters those out.
