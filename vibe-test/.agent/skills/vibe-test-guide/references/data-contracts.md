...
> This file adds the per-field, per-consumer notes that workflows and internal skills need to know
> at call time.
...
- `.agent/skills/vibe-test-decay/SKILL.md` via `stamp()` — refreshes `_meta.<field>.last_confirmed` when the user re-confirms a preference.

**Who reads (and when):**
- Every workflow at entry → persona, experience level, mode.
- `.agent/workflows/vibe-test.md` → greeting adaptation, returning-vs-first-run branching.
- `src/reporter/tier-adaptive-language.ts` → verbosity knobs per experience level.
- `.agent/workflows/vibe-test-audit.md` → initialize projects_audited counter on first audit of a repo.
...
### `~/.gemini/antigravity/data/vibe-test/profile.json`
...
### `~/.gemini/antigravity/data/vibe-test/sessions/<YYYY-MM-DD>.jsonl`

**Schema:** inline; see `session-log.ts` TypeScript types. Two-phase entry shape:

**Sentinel (written at workflow start):**
```json
...
**Terminal (written at workflow end):**
```json
...
**Who writes:** `src/state/session-log.ts` via `.agent/skills/vibe-test-session-logger/SKILL.md`. Sentinel at `start()`; terminal at `end()`. Both share a `sessionUUID` for pair-up.

**Who reads:**
- `.agent/workflows/vibe-test-evolve.md` → last 30 days of entries for proposed-change aggregation.
- `.agent/workflows/vibe-test-posture.md` → recent activity summary.
- `.agent/skills/vibe-test-friction-logger/SKILL.md` `detect_orphans()` → sentinels without terminal past 24h become `command_abandoned` entries.
...
**Who writes:** `src/state/friction-log.ts` via `.agent/skills/vibe-test-friction-logger/SKILL.md` at the trigger points declared in [`./friction-triggers.md`](./friction-triggers.md).

**Who reads:**
- `.agent/workflows/vibe-test-evolve.md` → last 30 days, weighted by confidence × trigger-map calibration.
- Future `/friction` read-only inspection command (v0.3).
...
### `~/.gemini/antigravity/data/vibe-test/wins.jsonl`
...
**Who writes:** `src/state/wins-log.ts` via `.agent/skills/vibe-test-wins-logger/SKILL.md`. Three capture moments (Pattern #14):
1. Absence-of-friction inference (applied by `/vibe-test-evolve` at aggregation time, batched — not inline).
2. Explicit success markers (on unambiguous positive user reaction).
3. External validation (cold-load, testimonial, screenshot).

**Who reads:** `.agent/workflows/vibe-test-evolve.md` to weight proposed changes — wins counter-balance friction when the plugin is working as designed.
...
**Who reads:**
- Every workflow at entry — returning-vs-first-run detection, last-run metadata.
- `.agent/workflows/vibe-test.md` — banner branching (first-run vs returning).
- `.agent/workflows/vibe-test-generate.md` — blocking prereq (audit-state must exist for current scope).
- `.agent/workflows/vibe-test-gate.md` — threshold baseline.
- `.agent/workflows/vibe-test-posture.md` — ambient summary source.
...
**Who reads:**
- `.agent/workflows/vibe-test-generate.md` → blocking prereq.
- `.agent/workflows/vibe-test-gate.md` → reuses if fresh (<1h old on same HEAD).
- `.agent/workflows/vibe-test-posture.md` → ambient read.
...
**Who writes:** every workflow at terminal via `src/state/beacons.ts`.
...
- `~/.gemini/antigravity/plugins/installed_plugins.json` — Pattern #15 resolution.
...
- The `vibe-*` workflows/skills available in this Antigravity workspace — composition dynamic discovery.
