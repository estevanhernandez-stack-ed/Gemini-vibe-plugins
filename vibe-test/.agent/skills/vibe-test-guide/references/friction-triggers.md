...
Source of truth for *"when does each Vibe Test command log which friction type"*. Every workflow references this doc in its "Friction Logging" section. The vibe-test-friction-logger skill reads from here at log time only via the calling workflow — this file is for humans and for future `/vibe-test-vitals` check #6, which audits the bidirectional consistency between this map and the actual `vibe-test-friction-logger.log()` invocations sprinkled across the workflows.

## How to read this file

Each section covers one command. Within a section, a markdown table lists every condition under which that command should call `vibe-test-friction-logger.log()`, the friction type it emits, the default confidence, and any required-field notes.
...
**`/vibe-test-evolve` weighting at high/medium/low:** `1.0 / 0.6 / 0.3`. Calibration entries can zero a row out post-hoc.

## Catalog-wide invariant

> When in doubt, don't log.

A missed friction signal is recoverable through manual `/vibe-test-evolve` calibration. A false positive corrupts `/vibe-test-evolve`'s weighting and is much harder to undo.

## Universal triggers (apply to every command)

| Trigger | Friction type | Confidence | Notes |
|---------|---------------|------------|-------|
| Sentinel session-log entry has no terminal pair after 24h (detected by `vibe-test-friction-logger.detect_orphans()`) | `command_abandoned` | high | Emitted out-of-band by router's first-run-of-the-day invocation. Per-command sections do **not** call this. |
| User asks the agent to re-explain a previous answer, AND the prior turn is captured in `symptom` as a quoted snippet | `repeat_question` | high | **Defensive default:** without a quoted prior in `symptom`, do not log. Better to miss than poison. The vibe-test-friction-logger module enforces this gate in `friction-log.ts` before it writes. |
...
`/vibe-test-evolve` is a reflection command — it reads the logs and proposes SKILL changes. It does not emit friction itself; proposal rejections are captured as `default_overridden` against the specific command the proposal targets, not against evolve.

---

## /vibe-test-vitals (internal)

`/vibe-test-vitals` is a self-diagnostic — checks that files, schemas, and references resolve. User declines on auto-fix prompts are the **expected** mode of interaction, not friction. Logging them would flood `/vibe-test-evolve` with noise about users simply choosing not to apply a fix. By spec scope, `/vibe-test-vitals` does not call `vibe-test-friction-logger.log()`. Only the universal `repeat_question` / `rephrase_requested` triggers apply, and only under the quoted-prior gate.
...
2. Pick the friction type from the canonical list in `src/state/friction-log.ts`. If none fit, that's a signal the type set itself needs revisiting — open an `/vibe-test-evolve` proposal rather than coining a new type silently.
...
4. Add the matching `vibe-test-friction-logger.log()` invocation in the workflow at the trigger point.
