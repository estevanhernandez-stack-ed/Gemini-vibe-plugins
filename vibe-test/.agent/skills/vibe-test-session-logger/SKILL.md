---
name: vibe-test-session-logger
description: "Internal SKILL — not a user workflow. Two-phase append-only session log for Vibe Test: a sentinel entry at command start (outcome=in_progress) and a terminal entry at command end, paired by sessionUUID. Referenced by the guide SKILL; invoked by every command at start and end. Part of Level 2 (session memory) of the Self-Evolving Plugin Framework."
---

<!-- Derived from vibe-cartographer 1.5.0 vibe-test-session-logger SKILL (own-impl per Spec Decision 5 / Option a; migrate to @626labs/plugin-core in Phase 3) -->

# vibe-test-session-logger — Sentinel + Terminal Session Log
...
- **Data contract:** [`../vibe-test-guide/references/data-contracts.md`](../vibe-test-guide/references/data-contracts.md) — read the "sessions" section. The sentinel vs terminal shapes, required fields, and sessionUUID pairing contract live there.
- **Schemas:** [`../vibe-test-guide/schemas/`](../vibe-test-guide/schemas/) — the session-log JSONL uses the inline shape documented in the TypeScript types at `src/state/session-log.ts`. No separate JSON Schema file in v0.2 — schema is per-entry via `schema_version: 1`.
- **Guide reference:** `AGENTS.md` + the `vibe-test-guide` skill (`.agent/skills/vibe-test-guide/SKILL.md`) — session memory interfaces section. This SKILL is the referent.
...
- **plugin_version** — read from `.agent/agent.json`'s `"version"`. Fall back to `"unknown"` if unreadable.
...
- Appends to `~/.gemini/antigravity/data/vibe-test/sessions/<today>.jsonl` via `appendJsonl()`.
...
3. **On write failure**, the helper swallows the error. The command's handoff line has already rendered; the user doesn't see a vibe-test-session-logger error.
...
- User-inspectable. `/vibe-test-posture` surfaces recent session counts; a future `/friction` will dump friction adjacent to sessions.
...
The **sentinel pattern** lets `vibe-test-friction-logger.detect_orphans()` distinguish *"user abandoned the command"* (orphan sentinel = `command_abandoned` entry) from *"command never ran"* (no entry at all — not friction).
